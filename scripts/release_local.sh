...
#                                    .env. Interactive wizard asks when unset.
#
...
pick_platforms() {
  ...
  picked="$(ui_checklist 1 1 "Platforms (one arch each — same as Actions)" -- \
    ...
    "android_tv_arm64|Android TV — arm64 only · Downloader code at R2|0" \
    "android_tv_armeabi_v7a|Android TV — armeabi-v7a only · Downloader code at R2|0")" || {
    rc=$?
    ((rc == 2)) && return 2
    ui_abort
  }
...
collect_android_tv_downloader_codes() {
  ...
  for arch in "${arches[@]}"; do
    case "$arch" in
      armeabi-v7a) label=ARMv7; url="${cdn}/latest/Forja-${ver}-android-tv-armeabi-v7a.apk" ;;
      arm64) label=ARM64; url="${cdn}/latest/Forja-${ver}-android-tv-arm64.apk" ;;
      *) continue ;;
    esac
...
wizard_release_tag() {
  local tag ver detail rc do_sync=1
  while true; do
    pick_platforms || {
      rc=$?
      ((rc == 2)) && return 2
    ...
    unset FORJA_HQ_MANIFEST_SOURCE _FORJA_HQ_MANIFESTS_READY || true
    pick_forja_hq_manifest_source || {
      rc=$?
      ((rc == 2)) && continue
      ui_abort
...
wizard_new_version() {
  ...
  while true; do
    ...
    do_backfill=0
    if ui_confirm_screen 3 7 "Backfill untagged commits first?" \
...
Mirror:  $([[ "$do_sync" == 1 ]] && echo "yes → ${SYNC_REPO}" || echo no)" 1; then
    ...
    *)
      ...
      case "$tool" in
        build_macos|build_windows|build_android_tv)
          unset FORJA_HQ_MANIFEST_SOURCE _FORJA_HQ_MANIFESTS_READY || true
          pick_forja_hq_manifest_source || {
            rc=$?
            ((rc == 2)) && return 2
