...
- Tap any tab to switch sections instantly (recently used tabs stay mounted for fast switching; Android TV keeps fewer tabs in memory, and opening the fullscreen player unloads other tabs — keeping the screen under the player — so playback gets max resources). While any fullscreen player is open (movies, trailers, Live Matches, IPTV), the nav rail stays mounted but Offstage (hidden) so exit does not remount it. On **Android TV**, details and player opens are an instant cut (no slide) so older sets do not stutter while the new screen loads.
