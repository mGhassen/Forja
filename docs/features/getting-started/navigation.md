...
- Tap any tab to switch sections instantly (recently used tabs stay mounted for fast switching; Android TV keeps fewer tabs in memory, and opening the fullscreen player unloads other tabs — keeping the screen under the player — so playback gets max resources). The nav rail stays mounted for root fullscreen players (movies, trailers, Live Matches) the same way as the underlay tab — only IPTV’s shell-overlay player hides the rail Offstage for full-width video. On **Android TV**, details and player opens are an instant cut (no slide) so older sets do not stutter while the new screen loads.
