...
- If every feature tab is off **and you are not already in Settings**, the shell shows a get-started screen with three cards (**Plugins**, **Addons**, **Features**) (sidebar logo hidden; profile avatar stays). Short subtitle: pick Plugins, Addons, or Features. Side by side on desktop/TV (stacked on phone). **Desktop** centers on the full window; **Android TV** centers in the body beside the nav rail with cards spanning that width. D-pad ←/→ across cards (← from the first card returns to the nav rail). Unchecking the last Features tab while already in Settings keeps Settings open — no empty get-started flash.
