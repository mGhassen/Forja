...
- **Desktop:** same D-pad / arrow-key focus graph as Android TV (focus rings, shelf ←/→, left-to-nav), plus normal mouse hover. Layout density and rail chrome stay desktop; hero Ken Burns pan/zoom stays on (static stills remain Android TV–only). Player **Sources** / **Episodes** stay right-side panels like details — not the leanback centered dialog
