...
- With **Cards** + **Detail page**, browse landscape match cards and open **hero details** (Providers / Live TV in a two-column stream grid on desktop — left column fills first; soft frosted scrim from the title to the bottom, full width to the screen edges; shell navbar stays visible). Same streams as the side panel — **Reload** sits at the far right of the tab row (clears the list and rediscovers, skipping warm cache); Live TV keeps the search icon next to the tab pills and the Categories rail when the panel is wide enough. On **Android TV**, D-pad **↓** from **Providers** / **Live TV** moves into the stream list; **↑** from the first stream returns to the tabs. **Cards** + **Side panel** also works — selected card chrome highlights while the panel is open.
