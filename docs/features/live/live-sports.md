...
- On the **Live Sports Cards** tab, browse a **landscape match card** grid and open **hero details** (Providers / Live TV in a two-column stream grid on desktop — left column fills first, then right; streams sit in a dark panel under the title; shell navbar stays visible). Same streams as the side panel.
