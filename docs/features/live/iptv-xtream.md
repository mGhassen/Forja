...
- Catalog is **cached in-session** per portal/section — switching chips reuses the last fetch. Login, Live/Movies/Series catalogs, and series episodes are fetched and normalized in the Rust engine (Flutter only renders)
...
- Browse live channels by category — channel tiles and compact list rows show logos fitted inside the card (not cropped to fill), plus a play control that floats upward with a slowly pulsing play icon, and a slightly brighter border, on hover (desktop) or focus (TV). On **Android TV**, live logos sit in a centered square band above the title (Movies / Series keep poster cover)
...
- On **Live**, press and drag any portal group row to reorder the category list (Favorites / Already watched stay fixed on top). Order is remembered per portal on this device when Sort → Categories is **Playlist Order**. Hover (or TV-focus) also shows a **pin** control that jumps a group to the top of the movable list; Movies / Series have no drag/pin
...
- Favorite portals in the Portals panel (star icon) — pinned to the top with **gold** title text matching the star; below them, the most recently scraped, dealt, or added portals appear first (with a **NEW** badge on session-fresh rows until you hover or focus them — styling only, position stays put)
...
- Portal rows show the subscription **end date** on the first line (green · yellow · amber · red by time left) and **connection seats** (`used/max`) under the server URL (blue when free capacity, gray when full); the **status dot** on the top-bar portal control (and panel rows) probes when a portal becomes active — green/red stay cached about **two minutes**, then the active portal is re-checked automatically; each successful probe also refreshes **expiry** and **used/max** (saved on this device); **Reload** on a shelf also re-checks; hover/focus on idle panel rows still probes when uncached; hover/focus reveals row actions (**copy share code / edit / delete**) without moving the status icon or star off-center
...
- Play in the IPTV player screen — opens in the shell overlay so **Back** returns to the catalog; the shell nav rail hides while the player is open (full-width video on TV/desktop). On **Android TV** playback uses the native ExoPlayer engine (reliable video surface); other platforms use MediaKit (libmpv). On **Windows**, live IPTV uses software decode (avoids a ~20s last-frame freeze with GPU decode). Volume (including mute) is remembered on this device and reapplied after reconnects. If the feed drops, the player shows **Reconnecting…** and retries. With controls visible, D-pad **←/→** moves between chrome buttons (back, play, mute, guide, fullscreen, …); **↑** from the transport row returns to **back**; **↓** from **back** returns to **play**; **OK** activates the focused control. When controls are hidden, **←/→** seek (VOD) and **↑/↓** adjust volume. On desktop, hover the volume icon to expand the volume bar (tap mutes; long-press pins the bar). The in-player **source** sheet lists are D-pad focusable; category **pin** is mouse-only on TV
...
- Section chips (**Live** / **Movies** / **Series**) switch the catalog; data stays cached until you hit **Reload** on the selected shelf (or the centered **Reload** button when the catalog failed to load — autofocused on **Android TV**)
