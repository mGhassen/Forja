...
- Switch **Live**, **Movies**, and **Series** from the pack top bar (full shelf on wide windows). Labels stay white on hover and focus. On a **narrow** window the shelf collapses to the selected section — hover (or TV focus) expands the other options and temporarily hides trailing chrome. While a shelf loads, the grid shows a centered spinner with **Loading channels/movies/series** (not placeholder cards). The app keeps the portal catalog on device and only paints one page of titles at a time (scroll for more). Channels appear as soon as the first page returns; **NOW / NEXT** EPG fills on each card afterward (not before the grid).
- Browse the **category list** on the left and **channel cards** on the right (logo + title under the mark, **NOW / NEXT** EPG strip when short EPG is available; long-press a card for the programme list). Wide windows: top-bar **Search**, **Sort**, and **Cards** / **EPG**. Narrow windows: always-open **Search** above the categories instead (**Cmd/Ctrl+F** focuses it); Sort and view controls are hidden. Hover probes stream health (green/red border). Type a letter while the pointer is over the rail or channel grid to jump to matching names. On **Live**, the rail includes **Favorites** and **Already watched**; hover a category to **pin** it, or hold-drag to reorder (playlist order). On TV, hold **OK** ~2s on a category, then → to focus the pin; ↑/↓ reorders that category while it stays floating (one step per press). Star a channel on its card to add it to Favorites; recently played channels land under Already watched (last 30 on this device). **Favorites** and **Already watched** are their own Live channel pages (saved ids); empty lists stay empty until you star or play. Opening Live lands on your **last category** when you have one; otherwise the first portal group in rail order — playlist first, or the category you **pinned** or **dragged** to the top (not Favorites). Reopening also scrolls to the last-played channel (no autoplay). On TV, OK / → from the nav lands focus on that last-selected category; → from a category focuses the last selected channel when it is still in that list, otherwise the first channel; focus or letter-jump lights one channel card — no second “selected” border. Leaving the player lands on the channel you were watching. On TV, in Favorites / Already watched, hold **OK** ~1s to jump to that channel’s portal group without playing. Narrow windows use Sources-style rows — bordered tile, green left bar on focus/hover, health on the bar — not fat rounded cards; on leanback TV, channel logos load after a short idle so skimming categories does not thrash images. Switching categories (or Favorites / Already watched) and **Search** re-query the on-device shelf page for that group — they do not re-download the whole portal. **Sort** (wide) opens a menu with separate **Categories** and **Channels** orders (Playlist / A–Z / Z–A). **EPG** (wide) is the channel×time guide (sticky channel rail + programme blocks; desktop Live only). **Movies** / **Series** keep poster cards.
