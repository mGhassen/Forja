...
- Switch **Live**, **Movies**, and **Series** from the pack top bar — while a shelf loads, the grid shows a centered spinner with **Loading channels/movies/series** (not placeholder cards). The app keeps the portal catalog on device and only paints one page of titles at a time (scroll for more). Channels appear as soon as the first page returns; **NOW / NEXT** EPG fills on each card afterward (not before the grid).
- Browse the **category list** on the left and **channel cards** on the right (logo + title under the mark, **NOW / NEXT** EPG strip when short EPG is available; long-press a card for the programme list). Hover probes stream health (green/red border). Type a letter while the pointer is over the rail or channel grid to jump to matching names. On **Live**, the rail includes **Favorites** and **Already watched**; hover a category to **pin** it, or hold-drag to reorder (playlist sort). Star a channel on its card to add it to Favorites; recently played channels land under Already watched (last 30 on this device). Reopening Live restores your last category and scrolls to the last-played channel (highlight only — no autoplay). Leaving the player lands on the channel you were watching. On TV, in Favorites / Already watched, hold **OK** ~1s to jump to that channel’s portal group without playing. **List** view (and narrow windows) uses Sources-style rows — bordered tile, green left bar when selected, health on the bar — not fat rounded cards; on leanback TV, channel logos load after a short idle so skimming categories does not thrash images. Switching categories (or Favorites / Already watched), **Search**, and **Sort** filter the already-loaded catalog instantly — they do not re-fetch channels. **Sort** opens a menu with separate **Categories** and **Channels** orders (Playlist / A–Z / Z–A). Switch **Cards** / **List** / **EPG** from the top-bar view group — **EPG** is the channel×time guide (sticky channel rail + programme blocks; desktop Live only). **Movies** / **Series** keep poster cards.
