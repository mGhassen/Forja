...
- Play live channels and VOD in the live player (player opens immediately with Search / Guide chrome; the full channel list fills from the hub shelf in the background). Hover or focus a channel in the in-player guide or Search results to check live status (green/red). Progressive live (Xtream `.ts`) opens the CDN URL in MediaKit with ffmpeg reconnect — no local relay.
