...
- Hover / D-pad focus a **Providers** row with a playable URL for a short pause — grey then green/red left strip. Signed Streamed / WatchFooty / **Stremio** (Highfly, flixnest, …) probe with their addon Referer headers (not a bare HTTP check). Unresolved `pending` / embed-only rows skip the strip until unlock.
