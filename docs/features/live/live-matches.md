...
- Streamed / PPV embeds play inside a WebView. Forja wraps the embed in an iframe under the catalog site (`streamed.pk` / `ppv.is`) on **every platform** (including Android / Android TV) so `document.referrer` matches the website; ad scripts that block the player page are filtered. The catalog wrapper URL is allowed once so the player can paint, then main-frame ad redirects are cancelled so they cannot take over the player. Opening the embed URL as a top-level WebView page breaks the host lock and shows a red “Remove sandbox attributes on the iframe tag” message — that is not fixed by spoofing Referer headers alone.
