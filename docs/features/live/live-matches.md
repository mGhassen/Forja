...
- Switch between **card view** and **timeline view** with the button to the right of Refresh. Timeline view groups events into **30-minute** rows — streams that start in the same half-hour share one horizontal line of backdrop cards (swipe sideways for more; the time for that row stays put). The playhead sits near the top so the current row (even a single card) lines up with its time on the ruler. Use the **Day / 12h / 6h** buttons to set how many hours one screen height of the ruler covers. CDN channels have no schedule, so timeline shows CDN sport events only.
