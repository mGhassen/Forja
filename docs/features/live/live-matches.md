...
- Browse upcoming and live events — live matches appear first (Streamed also merges the site’s **live** feed with the full schedule, so Popular Live rows like ACA / day-long golf are not missing). Only **live** matches are tappable and show the play button (green and floated upward, with a slowly pulsing play icon, on hover/focus). Upcoming cards still highlight and lift on hover in the timeline; they show the start time badge only
