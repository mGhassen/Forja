...
Tap **Live Sports** in the navigation bar. The tab is a catalog hub (`plugins/hubs/live_sports`) — pack layout is generic `kit.list` with `source: live_schedule`. The schedule grid always uses the **Catalog** sources; **Forja Live**, **Forja Sports**, and **Stremio** streams show on match detail under **Providers** / **Live TV** (Settings toggles still control what is available).

## What you can do

- Use the **Catalog** button (label shows **PPV**, **Streamed**, **Sports Streams**, …) to filter the **schedule grid** — engine catalogs and each installed **Stremio** live addon appear as separate rows (addon display name). Opening a match still resolves every enabled provider in **Providers** on the detail page, not just the selected catalog. A **schedule** button next to that opens one sheet with **Status** (**Airing** / **Upcoming** / **Airing + upcoming**) and **Horizon** (**1h** / **3h** / **6h** / **24h**).
...
- On **TV (Android TV / leanback)**, Live Matches is **cards only** — no timeline view and no view toggle. Top bar is **Catalog** + **Schedule** (+ **Portals** when Forja Sports is enabled) + **Refresh**. Opening a match pushes the **detail** page; **Providers** merges Forja Live with Stremio; **Live TV** lists Xtream-matched channels. Match and channel tiles are **landscape**. Sport category circles are **centered** and scaled to fit when needed; hidden when only one sport is loaded. D-pad moves **Catalog → schedule → Refresh → Portals (when shown) → sport chips (when shown) → match cards**. **↑** from sport chips returns to the top bar. Focused Catalog / schedule use the same brand-green focus chrome as **Portals**; focused **Refresh** turns white. Activating **Refresh** keeps D-pad focus on **Refresh** after the catalog reloads. Within the card grid, **←/→** stay on the current row. **↑** from the first card row returns to sport chips when shown, then schedule / Catalog. **←** from the left edge of a row returns to the nav rail. Empty lists autofocus **Refresh**. Entering the tab restores the last grid focus
