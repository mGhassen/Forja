...
- Hover / D-pad focus a **Providers** row that already has a playable HLS/MP4 URL for a short pause — grey then green/red left strip (signed Streamed / WatchFooty rows probe with their Referer). Unresolved `pending` / embed-only rows skip the strip until unlock. **Stremio** Live TV rows (e.g. flixnest signed playlists) do **not** show a green/red hover status bar — a bare HTTP check false-fails while the player still opens after a few retries.
