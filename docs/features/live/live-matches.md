...
- Switch between **card view** and **timeline view** with the button to the right of Refresh. Timeline view groups events into **30-minute** rows — streams that start in the same half-hour share one horizontal line of backdrop cards (swipe sideways for more; the time for that row stays put). An animated ruler gauge on the left tracks the playhead as you scroll vertically through time. Use the **Day / 12h / 6h** buttons at the top to set how many hours one screen height of the ruler covers. CDN channels have no schedule, so timeline shows CDN sport events only.
