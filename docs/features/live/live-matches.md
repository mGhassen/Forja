...
Tap **Live Sports** in the navigation bar. The tab is a catalog hub (`plugins/hubs/live_sports`) — pack layout is generic `kit.list` with `source: live_schedule`. The hub shows the merged schedule grid (sport chips when needed) with **no top menu**. **Forja Live**, **Forja Sports**, and **Stremio** streams show on match detail under **Providers** / **Live TV**.

## What you can do

- Browse the **merged** schedule from every enabled live catalog (no Catalog / Schedule top bar). Opening a match still resolves every enabled provider in **Providers** on the detail page.
...
- On **TV (Android TV / leanback)**, Live Matches is **cards only** — no timeline view and no hub top bar. Opening a match pushes the **detail** page; **Providers** merges Forja Live with Stremio; **Live TV** lists Xtream-matched channels. Sport chips (when shown) sit above the grid. D-pad: **sport chips → match cards**; **↑** from chips/grid returns to the nav rail. Empty lists offer **Refresh**. Entering the tab restores the last grid focus
