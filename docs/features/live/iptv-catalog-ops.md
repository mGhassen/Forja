...
- **Deep refs** — all base64 / paste refs from scrape (`paste_url` only — body is never stored). Expand a row to fetch the paste live from the host (or decode inline base64), and see every portal found with **New insert** vs **Already in DB**
