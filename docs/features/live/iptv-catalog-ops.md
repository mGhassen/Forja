...
- **Deep refs** — flat table (no card chrome) of all base64 / paste refs (`paste_url` only — body is never stored), with search/status filters + pagination. **One row per paste** (`post_id`+hash) — Scrape **L2 portals** are hits *inside* those pastes, not extra deep-ref rows. Force-full re-upserts the same row (**Last** = `updated_at` / run start; first-seen stays on hover). Expand a row for portal hits (paginated) with **New insert** / **Already in DB** / **Not promoted** (promoted rows link to **Pool** and highlight that portal). Pool rows link back via `?ref=` (filters + expands that deep ref). **Show** opens a live paste body panel. **Reprocess** re-fetches the paste (or re-decodes base64), re-extracts portals, replaces junction rows, and promotes into the deal pool (paste fetch failure leaves existing portals unchanged). **Backfill promote** recovers stranded hits (`portal_id` null but eligible): dialog shows pending count + limit presets (100 / 1 000 / 5 000 / All); Inngest `iptv-promote-backfill`. **Backfill stalker notes** re-fetches pastes for Stalker hits missing scrape expires (`null` / empty / `Unknown`) and writes `note` + `expiry` on junction / pool rows; dialog **Eligible deep refs** is a paged distinct-paste count (not total deep refs, not pool portals); Inngest `iptv-stalker-note-backfill` — live chips **Deep refs · Paste ok · Paste fail · Junctions · Portals**. Start syncs the serve endpoint then fires the event. Live **running** panels + Stop on `iptv_scrape_runs`.
- **Pool** — lists **all** `iptv_portals` (paged past PostgREST’s 1000-row cap). Filter **Inventory** (All / Deal pool / Not in pool; default All), **Type** (All / Xtream / M3U / Stalker), **status**, and **region**. Catalog-pool rows show a **pool** badge; trash removes from catalog pool only (row stays). Host table (host / accounts / alive / scraped) with pagination — column sort applies to **hosts only** (re-sort only when you click a column, not after Check status). Expand for portal grid (card order and scroll stay fixed while you check status; host Alive counts update in place). Cards show **Ends:** from `expiry` and **Note:** from scrape expires (`note`, Stalker paste) when present. Promoted portals show **Deep ref →** (opens Deep refs focused on the scrape lineage). **User+** opens who has that portal + assign/unassign. **Check status** probes by platform (Xtream `player_api`, Stalker handshake/`get_profile`, M3U playlist fetch) and updates alive/expiry in place — Xtream is alive only when `auth=1` or `status=Active` (same as Forja; a bare `user_info` object is not enough). Does not clear scrape `note`; does not upsert or re-add to the deal pool. Header link jumps to **Scrape** for start/stop
- **Scrape** — start/stop/mark stuck + full run history (all runs, paginated; realtime + fast poll). **Run normal** = only new posts since last watermark. **Run full** = in-app dialog to pick page count (shows ≈ posts; 10 posts/page), then ignore known posts and re-extract. Schedule under Automation. **Backfill promote** / **note backfill** jobs also land here (`source` = `promote-backfill` / `stalker-note-backfill`)
- **Providers** — edit remote provider runtime config (templates, APIs, source host bases, anime hosts/mirrors, CDN Referer rules) via flat editable tables (each paginated) or raw JSON
...
4. Reddit listing today is **`r/IPTV_ZONENEW`** (other old catalog subs are banned). Posts are usually base64 → encrypted **paste.sh** links — scrape decrypts those (L2), then runs **mechanical** credential extract (get.php, status cards, `host:port user:pass` dumps, Spanish panel **Portal / User / Pass** column sheets without a colon between creds, stalker `Portal`/`MAC Addr`/`Exp date` cards). Optional LLM agent is **off unless** `IPTV_LLM_EXTRACT=1` + API key (and never fails the run). Every platform upserts into the deal pool; deep-ref rows keep **`type`** + full **`output`**. **alive** stays null until Pool → Check status.
...
- `20260826010723_…` — `iptv_portals.note` + deep_ref `note` (Stalker scrape expires); upsert `p_note`; pool RPC returns note
