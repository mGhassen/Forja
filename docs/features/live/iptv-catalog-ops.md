...
6. **Two phases** — **Collect:** Reddit pages → posts + deep_ref stubs (base64 + paste_url, no paste HTTP). **Process:** load pending stubs from DB → fetch pastes + extract portals (one Inngest step per ref). Avoids gateway 504s from Reddit+pastes in one request. Full scrape dialog: presets `1–N`, or custom range `5-10` (skip to 5th page, scrape through 10th).
