...
| **`profile_settings` → Features** | Visible shell tabs and default tab — editable on web under **Profile → Features**. Empty list means no feature tabs (Settings only), same as a fresh app install — the portal must not display that as every tab on. |
