...
| **`profile_settings` → Playback** | Player prefs (audio / subtitle, auto next/skip, content warnings, auto PiP, max quality, anime title language) plus play-source masters (torrent / Stremio / Nuvio) and **Addons IPTV unlock** (`addon_feature_iptv`). Web **Profile → Addons** hub owns the play-source + IPTV switches; **Addons → Playback** is player prefs only. IPTV EPG is under **Addons → IPTV**. Webstreaming / Simple resolve keys still sync from the app but are not editable on the portal. |
