...
- Set **IPTV live recovery** when the IPTV tab is visible — **Auto** by default (buffer-aware for every source: reconnect only when the live buffer is empty; no per-channel change). Or pick **Stable — buffer-aware** and optionally **Reopen on buffer stall**, or **Classic — stall timers**. Applies the next time you open the IPTV player
