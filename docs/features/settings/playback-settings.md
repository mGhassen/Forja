...
- Set **IPTV live recovery** when the IPTV tab is visible — **Auto** by default (manages stall for you: Xtream gets stall reopen, Stalker / Forja Live / Stremio keep buffer hold without stall). Existing installs that still had the old stall default are moved to Auto once on upgrade. Or pick **Stable — buffer-aware** and optionally **Reopen on buffer stall**, or **Classic — stall timers**. Applies the next time you open the IPTV player
