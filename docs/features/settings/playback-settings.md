...
- **Webstreaming** appears only for **admin** accounts (`is_admin`). When on: toggle **Simple resolve (experimental)** (on by default) — tries one provider at a time in **Tries** order, filters/probes streams, then opens the player once (old multi-provider race stays when this is off)
