...
- **Source scoring** tables: drag sets your **baseline** order per type. **Domain score** may move a provider at most **±2** positions before checking. **Effective** shows the pre-check order the engine uses. Failed checks in the player Source panel apply a persisted reliability penalty on top of domain score (badge + next open sort). Stream quality (codec, resolution, latency) is scored **after** resolve — not shown in settings.
