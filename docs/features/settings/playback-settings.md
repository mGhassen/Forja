...
- **Server reliability**: tabs for Movies / Series / Anime / Asian Drama (one list at a time). Drag to prefer a server. **Score** rises when that server works across titles you play. **Tries** (1st, 2nd, …) is the order Auto tries them. Asian Drama currently lists **KissKH** only (single source). In the player Source panel, the **badge number** is the same Score; **+/−** prefixes are this film/episode only (see [Stream providers](../sources/stream-providers.md)). Stream quality (codec, resolution, latency) is scored **after** resolve.
