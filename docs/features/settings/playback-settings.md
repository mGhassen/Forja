...
- Set **IPTV live recovery** when the IPTV tab is visible — **Auto** by default (manages stall for you: Xtream gets stall reopen, Stalker / Forja Live / Stremio keep buffer hold without stall). Upgrade migrates unset / old stall / Stable-buffer to Auto once (Classic left alone). Or pick **Stable — buffer-aware** and optionally **Reopen on buffer stall**, or **Classic — stall timers**. Applies the next time you open the IPTV player
...
- On **Android TV**, **IPTV live buffer** is shown only to **admin** accounts — MediaKit only; **Auto (by resolution)** (default) uses HD 15 s / FHD 20 s / UHD 30 s with matching demuxer RAM, or pick **15 / 20 / 30 seconds** to force the cushion (also scales demuxer bytes). For underrun / RAM experiments — does not fix 50 fps-on-60 Hz judder. Takes effect the next time you open the IPTV player
