...
- **Green Play** (hero play icon) follows each hub pack’s **Green Play** settings (technologies + providers: allowlist, preferred, order) — not the Sources chip selection. Edit under **Addons → Anime** (and other pack Addon pages) or **Forja Packs** expand for hubs that declare it. Hubs without a declaration race all enabled Forja providers.
