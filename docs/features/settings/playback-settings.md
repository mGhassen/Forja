...
- On **Android**, choose **Built-in engine** — **ExoPlayer (Media3)** (default on phone and for Android TV **movies/series**) or **MediaKit (libmpv)** — in Settings for **movies/series**, or from the **Player** button in playback controls. **IPTV** follows the movies engine until you change it in-player. **Live Matches** defaults to **MediaKit**. Each surface remembers its own in-player choice. On **Android TV**, the in-player **Player** menu only lists those two engines (no external apps)
...
- **Built-in engine** (Android only) applies to **movies and series** when Video Player is **Built-in** — desktop/iOS always use MediaKit; **IPTV** follows the movies engine until you change it from the in-player **Player** menu; **Live Matches** defaults to **MediaKit** until you pick Exo from that same menu
