...
- On the **Forja** play-source row: **Auto** checkbox (before the switch) — when checked and **Webstreaming** is off, green **Play** / **Resume** on movies/TV **and Asian Drama** races every enabled Forja HTTP plugin (drama-tagged set on Asian Drama) and opens the first stream. Off = Forja only via **Sources → Forja**. Ignored while **Webstreaming** is on (Asian Drama then uses KissKH native green Play)
