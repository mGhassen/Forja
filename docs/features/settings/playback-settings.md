...
- On **Android**, choose **Movies & series engine** — **MediaKit (libmpv)** (default) or **ExoPlayer (Media3)** — for Home, Search, Anime, Asian Drama, and **IPTV Movies/Series** (also changeable from the in-player **Player** menu). When the IPTV tab is visible, **IPTV engine** sets **live channels** only (independent of Movies). **Live Sports** defaults to **MediaKit** and remembers its own in-player choice. On **Android TV**, the in-player **Player** menu only lists those two engines (no external apps)
...
- On **macOS / Windows** (desktop profile only — **not** Android TV or phone): toggle **In-app mini player** (**off** by default). When on, Escape (after hide chrome) or the top-right mini button shrinks the player to a corner **inside** Forja so you can browse — playback keeps going. Not the same as picture-in-picture. See [In-app mini player](../playback/in-app-mini-player.md)
...
- **Movies & series engine** / **IPTV engine** (Android only) are independent — desktop/iOS always use MediaKit; **MediaKit** is the Android default (ExoPlayer is optional); **Live Sports** defaults to **MediaKit** until you pick Exo from the in-player **Player** menu
