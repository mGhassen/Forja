...
- On **Android**, choose **Built-in engine** — **ExoPlayer (Media3)** (default on phone and Android TV) or **MediaKit (libmpv)** — in Settings for **movies/series**, or from the **Player** button in playback controls. **IPTV** remembers its own in-player choice separately; **Live Matches** defaults to **MediaKit** until you change it from the in-player **Player** menu. On **Android TV**, the in-player **Player** menu only lists those two engines (no external apps)
...
- **Built-in engine** (Android only) applies to **movies and series** when Video Player is **Built-in** — desktop/iOS always use MediaKit; **IPTV** on Android keeps its own Exo / MediaKit choice from the in-player **Player** menu (ExoPlayer default until you change it); **Live Matches** defaults to **MediaKit** until you pick Exo from that same menu
