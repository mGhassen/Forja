...
- Set **IPTV live recovery** when the IPTV tab is visible — **Stable — buffer-aware (1.3.170)** by default (only reconnect when the live buffer is empty) or **Classic — stall timers (1.3.114)**. **Reopen on buffer stall** is on by default and shown only to **admin** accounts — reconnects when buffering/freeze stalls with no playhead. Applies the next time you open the IPTV player
...
- Play source toggles: green **Play** (play icon) uses **Webstreaming** extractors only (admin); white link **Play** / **Sources** use **Direct torrent**, **Stremio**, **Nuvio**, and **Forja** when each is enabled (see [Webstreaming](../movies-tv/direct-streaming-mode.md)). Fresh installs default to **Forja** only. The **Sources** panel uses **Forja / Torrents / Stremio / Nuvio** (left to right) for the play sources you turned on.
