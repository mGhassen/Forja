...
- Anime uses the same resolve + Rust scoring pipeline as movies; saved source pin still wins when set. Default Anime **Tries** order starts with Megaplay (direct embed) before Miruro Cloudflare pipes — Reset in Server reliability restores that if you customized the list
