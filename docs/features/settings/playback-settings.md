...
- Enable **Play sources**: **Direct torrent** (**All** / per-provider chips / Jackett / Prowlarr in **Sources**), **Stremio**, **Nuvio**, and **Webstreaming** (torrent / Stremio / Nuvio / webstreaming on by default on phone and desktop; **Android TV** shows **Webstreaming** always, and **Direct torrent / Stremio / Nuvio** only after you pair a desktop under **Settings → LAN** — then they play via that desktop; **Sources**, **WebStreamr**, **Debrid**, **Lists**, and **Data & backup** stay hidden on TV)
...
- On **Android**, choose **Built-in engine** — **MediaKit (libmpv)** (default on phone and Android TV) or **ExoPlayer (Media3)** — in Settings for **movies/series**, or from the **Player** button in playback controls. **IPTV** follows the movies engine until you change it in-player. **Live Matches** defaults to **MediaKit**. Each surface remembers its own in-player choice. On **Android TV**, the in-player **Player** menu only lists those two engines (no external apps)
...
- Set **IPTV live recovery** when the IPTV tab is visible — **Stable — buffer-aware (1.3.170)** by default (only reconnect when the live buffer is empty); **Stable — reopen on buffer stall (test)** keeps that hold but reconnects when buffering/freeze stalls with no playhead; **Classic — stall timers (1.3.114)** reconnects on freeze/buffering timers. Applies the next time you open the IPTV player
...
- **Built-in engine** (Android only) applies to **movies and series** when Video Player is **Built-in** — desktop/iOS always use MediaKit; **MediaKit** is the Android default (ExoPlayer is optional); **IPTV** follows the movies engine until you change it from the in-player **Player** menu; **Live Matches** defaults to **MediaKit** until you pick Exo from that same menu
