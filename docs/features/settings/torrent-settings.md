...
- Enable or disable each **torrent provider** after you install the **ForjaHQ Torrent** pack under Settings → Sources → Forja — until then, Settings shows an install hint instead of provider toggles. Enabled providers show as chips under **Sources → Torrents** (plus **All**) when **Direct torrent** is on and built-in torrent search is available on this platform. Paired LAN clients search on-device via the same pack (the desktop LAN server only streams magnets, it does not run indexers).
