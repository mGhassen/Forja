...
- Toggle **Select All by default** under **Forja plugins** — on opens Sources → Forja with enabled plugins for the current media category selected (Movie on movies, TV on series); off starts with none. Anime / Drama plugins stay available from Filters → Category. Expand a pack and use tabs **Movie & TV** / **Anime** / **Drama** (one list at a time). **ForjaHQ** auto-installs on first launch; paste another `manifest.json` URL under **Add plugin** for community packs. Use **Refresh** / **Remove** per pack. Each HTTP plugin carries its own **config** bag in that pack (API bases, mirrors, keys — not shared across plugins). **Admin → Providers** remote JSON can overlay `engine.<pluginId>` keys at runtime (nested maps merge; mirror lists replace) without reinstalling the pack. **Videasy** probes every [player.videasy.to](https://player.videasy.to) server (Yoru, Cypher, Breach, Neon, Vyse, Killjoy, Fade, Omen, Raze) and lists each hit as its own row. When a mirror only returns an HLS master, Forja expands it into 1080p / 720p / 480p rows (same card as Nuvio: title + episode + year, quality badges when the playlist has them).
