...
- **Reload** only when at least one pack is fully installed — re-downloads every installed pack’s manifest and scripts. While it runs, the pack list stays put (busy **Reload** + “Reloading packs…”) so rows don’t flash or collapse
