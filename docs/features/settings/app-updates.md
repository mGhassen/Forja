...
- On **macOS**, download the `.dmg`, then choose **Install and close Forja** to apply the update into your installed app and reopen Forja (no Finder / DMG drag). If macOS blocks opening the new build, you get a system dialog with how to allow it, and a toast on the next launch if the update did not apply. If Downloads is unavailable, the app falls back to its own updates folder
