...
On a fresh install, all available tabs are enabled in this order: Home, Asian Drama, Anime, IPTV, Live Matches, My List, then Settings. Home is the default startup tab.
...
- Toggle visibility for each available tab (Home, Asian Drama, Anime, IPTV, Live Matches, My List) — on **TV**, **OK** on the tab name toggles visibility (same as the switch on desktop)
...
Some hubs (Search, Discover, Similar, Media Downloader, Magnet, Audiobooks, Books, Music, Comics, Manga, Jellyfin, Anime Arabic, Arabic) are temporarily withheld from this list and the shell — code remains; they are not deletable from Settings either.
