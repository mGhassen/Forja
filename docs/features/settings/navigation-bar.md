...
- Sync the same layout across devices via [cloud sync](cloud-sync.md) (visible tabs + default tab + Addons feature flags). Web **Profile → Addons** writes the same IPTV / Live Sports unlock flags as the app. Edits wait until they are saved locally before syncing, so a toggle does not snap back off when the window refocuses.
