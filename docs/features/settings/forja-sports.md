...
- **Catalog** — schedule toggles (Streamed, PPV, TimStreams, StreamFree, WatchFooty, Streamic, ESPN). Checked catalogs appear as chips on Live Matches **All** / **Forja Live** and load schedule rows. **Live plugins** toggles are for stream resolve only.
