...
- **Desktop:** focus the search field and type; press **Enter** / IME Search to run search (typing alone does not query). **Android TV:** focus highlights only — **OK** to type, then **OK** / IME Search again to run search. Your last 5 searches stay above up to 16 recommendations. Idle uses a varied catalog pool; while searching, recommendations refresh from the top match (similar titles, genre, year, language, director/creator, studio/network) and never list the same titles as the result cards. Result cards on the right mix **FILM** and **TV** in TMDB relevance order (not all movies first). Tap the **X** on a recent search to remove it. Tap a recent or recommendation to search that title immediately. On **Android TV**, a search is saved to recent only when you press **OK** / submit. On desktop, click a card to open details (same as Home poster cards)
