...
- Browse featured and trending movies and series — each title appears in **at most one** discovery rail (hero → Featured → Popular → mood → Because → Trakt recs → New Releases → genre rows). When a title is already shown higher up, the lower row **refills** from a longer TMDB pool so the row stays full. Continue Watching and Trakt upcoming calendars can still show titles you are mid-watch or waiting for.
