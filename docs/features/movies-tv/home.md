...
- Search from the Home top-bar **Search** tab or **Cmd+F** / **Ctrl+F** — same hub kit search overlay. The Home pack declares `host_search` so results use the host engine (structured TMDB + Stremio addon catalogs) with the tune filter lens (type, score, year, genre, country, **language**). Filter-only Search (no typed title) works after you submit the lens. TMDB loads one page first; scrolling loads more pages up to five (skeleton cards while the next page loads). Anime / Asian Drama stay pack-only title search.
