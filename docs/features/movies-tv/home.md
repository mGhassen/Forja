...
- Search from the Home top-bar **Search** tab or **Cmd+F** / **Ctrl+F** — same hub kit search overlay. The Home pack declares `host_search` so results use the host engine (structured TMDB + Stremio addon catalogs) with the tune filter lens (type, score, year, genre, country, **language**); Anime / Asian Drama stay pack-only title search.
