...
- **Top-left back chevron** — muted until hover or D-pad focus (then white); tap / click / TV **OK** pops the shell overlay back to the previous screen. On **desktop**, the mouse **Back** side button, **Escape**, and (on macOS) trackpad swipe-between-pages (left-edge two-finger swipe, or the system swipe gesture) do the same. From the leftmost hero action (**Play** / **Resume**), **↑** focuses the back chevron and **←** moves focus to the shell nav rail (same as Home hero); **→** on the active nav tab returns focus to the details page.
