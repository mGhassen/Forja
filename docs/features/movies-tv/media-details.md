...
- Filter Sources by provider (Filters → Providers), quality, size (`<1 GB` · `1–3 GB` · `3–8 GB` · `8–20 GB` · `20 GB+`, multi-select OR), language, tech, and audio; under Nuvio, every enabled scraper starts selected in Providers, and **Load next provider** runs another selected scraper
