...
When **Direct streaming mode** is on (Settings → Playback), tapping a movie or series opens the **streaming details** screen. Forja automatically searches VidLink, VixSrc, WebStreamr, Nuvio scrapers, and other configured providers for playable links — no torrent list. Episode watched state is still tracked.
...
- Turn streaming mode off anytime to return to torrent-based details
