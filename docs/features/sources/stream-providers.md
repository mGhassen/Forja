...
Movie/TV embed **URL templates**, WebStreamr **bases**, anime **hosts / APIs / mirrors**, and **CDN Referer rules** can be overlaid from cloud `provider_runtime_config` (RFC-039) so ops can retarget URLs without shipping a new build — edit in Admin → Providers. Extract logic (decrypt, Cloudflare pipes, WebView sniff) still ships in the app.
...
- The **score badge number** is the provider’s **global Σ** (sum of title totals for that server, never below **0**). The **+/− prefixes** are outcomes **for this film / TV episode / anime episode** only: server **±2**, stream **±2** (both ok → **+4** on this title; both fail → **−4** and Σ drops by 4, floored at 0). Asian drama is not scored. Those per-title totals feed Σ and nudge Auto order over time.
