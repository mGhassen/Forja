...
- **VidSrc.sbs** (`vidsrc.sbs/embed/…`) is separate from **VSEmbed** (`vsembed.su`) and **VidSrc** (`vidsrc.win`). Its aggregator rejects iframe-wrapped playback, so Forja opens it as the headless WebView's top-level document before sniffing on desktop/mobile. On resolve (loading screen), Forja reads the embed’s server list and sniffs each mirror top-level in order — **PRO Multi**, **Cinesrc**, **Vlux**, then **Star** — instead of relying on the page’s Server dropdown. Nested proxy responses are still scanned for `.m3u8` in the response body. Policy lives under `shared/extractors/providers/vidsrcsbs/`
