...
**Sources → Forja** runs `extract(ctx)` plugins in parallel (EngineJS job pool). Install via **Settings → Forja Packs** or web **Profile → Forja Packs**. Plugins are tagged **movie** / **tv** / **anime** / **drama**. Settings groups **Movie & TV** / **Anime** / **Drama** / **Live** / **Catalog**. **All** runs up to **10** plugins in flight (**5** on TV); rows appear as each finishes.

Cold start (and profile switch) downloads any cloud-synced packs that are missing onto the device — progress shows in the splash’s normal bottom status line (pack name only) with a thin progress bar while packs install. If download is slow, stuck, or fails, **Continue in background** opens the app while install keeps going (autofocused on Android TV). Mid-session, when another device adds or removes a pack on your profile, Forja asks before downloading or uninstalling here. Mid-session installs also use a bottom progress card. **Settings → Reload** on a pack always re-fetches that pack; **Remove** deletes its cached scripts and pushes membership off the profile.
...
- **111477** / **DahmerMovies** use the `st.111477.xyz` Stremio addon (direct `workers.dev` play URLs). Cloudflare may rate-limit after a few hits — the plugin waits ~7s and retries instead of returning empty.
...
- **VidNest** ([vidnest.fun](https://vidnest.fun/)) resolves via `new.vidnest.fun` in bounded parallel — same server set as the site player (Lamda, Zeta, Filxer, Ophim, Prime, Alfa, Beta, Sigma, Gama, Catflix, Hexa, Delta). Movie/TV opens use **User-Agent only** unless the API sent Referer/Origin — Forja does **not** force `vidnest.fun` onto CDN playlists. Legacy MovieBox CDN links (`*.hakunaymatata.com`), if any mirror still returns them, still strip Referer/Origin (HTTP 429 when set). **Sources → Forja currently does JS/API only for debugging**: if the API path misses, it does not fall back to sniff in that panel.
- **VidRock** (`vidrock.ru` AES-GCM locally): Forja tab GETs `/api/movie/{tmdb}` or `/api/tv/{tmdb}/{s}/{e}` and decrypts each server URL — Astra playlists expand to quality rows. Atlas-style CDNs that wrap MPEG-TS in a PNG shell open through the local HLS strip proxy. **Sources → Forja currently does JS/API only for debugging**: green Play can still sniff `vidrock.ru/movie|tv/…`, but the Forja panel will show only the JS/API result.
...
- **VidUp** (`vidup.to/movie|tv/…`): Sources → Forja loads the title page **without** `X-Requested-With` (nginx 403s the HTML when that header is set), then uses `enc-dec.app` `enc-vidup` / `dec-vidup` for servers + stream POSTs (XHR + CSRF).
- **1Shows**: Sources → Forja uses `api.viduki.net` download-token + AES-GCM decrypt with Origin/Referer `www.1shows.bz`, then resolves HubCloud / direct hosts to playable URLs.
- **HDHub4u**: Typesense search via `search.pingora.fyi`, title pages on `new6.hdhub4u.cl`. HubCloud drives use `hubcloud.ist`; quality gates (`greenmotors` → `hblinks`) are followed to HubDrive / HubCloud file hosts.
- **DVDPlay / MalluMV**: Same catalog on `mallumv.space` (`search.php` → `/movie/` → `/internal/`); quality rows list Pixeldrain and HubCloud (`hubcloud.ist` → `hubcloud.php` → R2 / PixelServer). No `/confirm/` step.
- **MoviesMod**: Catalog on `moviesmod.ai.in`; quality rows open `links.modpro.blog` then unlock SID gates (`thenaukriadda` / linkpilot) to Driveseed Direct Links (`workers.dev`).
- **NetMirror**: Sources → Forja uses `net27.cc/api/embed-tmdb/{tmdb}` first (`?type=tv&se=&ep=` for episodes; play Referer `videodownloader.site`). NewTV OTT fallback discovers via `check.php` / `checknewtv.php` and fails fast when those backends are down.
- **MovieBox** (aoneroom app API): Sources → Forja visitor-logs in, searches by title, then reads `play-info/v2` (with `resource` fallback) across rotating API hosts. Streams open with the API Referer + any `signCookie` the CDN returns.
- **StreamFlix** (`api.streamflix.app`): Sources → Forja matches the catalog (`data.json`) by TMDB id, then builds direct file URLs from `config-streamflix2.json` CDN bases (`s1` / `s2` / `s3.streamflixserver.site`). Movies use each title’s `movielink`; TV uses `tv/{moviekey}/s{N}/episode{M}.mkv`.
- **AllMovieLand**: Catalog on `allmovieland.art`; IndStream host from `allmovieland.link/player.js` (and site `player.js`). Sources prefers IMDB `/play/{tt…}` then title search (`?do=search&…&story=`). Streams need a live IndStream embed — see [issue 341](../../issues/341-[open]-allmovieland-stale-base-cf-search-indstream-404.md).
- **Vidzee** (`player.vidzee.wtf`): Sources → Forja hits `core.vidzee.wtf/streams/…?e=0` across the player chips (**Dcloud**, **Apre**, **TCloud**, **IPcloud**, Hindi v3, Acme `v4:<lang>`). Playback uses API `headers` when present (CDN Referer). Dead playlists are omitted. Green Play can still sniff the embed when that panel uses WebView.
- **VidSrc.sbs** (`vidsrc.sbs/embed/…`) is separate from **VSEmbed** (`vidsrc.sh`) and **VidSrc** (`vidsrc.win`). Its aggregator rejects iframe-wrapped playback, so Forja opens it as the headless WebView's top-level document before sniffing on desktop/mobile. On resolve, Forja reads the embed’s `CFG.servers` list (site order) and sniffs **every** nested mirror in bounded parallel (two at a time). Nested **4K** / `player.videasy.to` / `player.videasy.net` URLs skip sniff and use the same STREAMCRYPTO HTTP path as Videasy. Other nested players rotate their own Servers chips (e.g. Pro Multi internals) and keep every playlist that emits — Sources lists them (e.g. `PRO Multi · …`, `Cinesrc`, `4K`). Playback opens the first reachable stream. Nested proxy responses are scanned for `.m3u8` in the body; Cinesrc-style `ice…/?m3u8=<token>` URLs are treated as playable HLS even without a `.m3u8` path. **Sources → Forja** uses the `vidsrcsbs.js` plugin: balanced-bracket `CFG.servers` parse, nxsha AES `/api/servers`+`/api/sources`, Videasy nests via STREAMCRYPTO.
...
- **2Embed** / **MultiEmbed** Sources → Forja use `multiembed.js` (`www.2embed.cc`). Live paths are **swish** (`2vcdn.skin` packed HLS) and nested **vidsrc.buzz** / **videm.xyz** players (`api.php?a=play` → `/_stream`). xpass is tried when listed but often empty. Dead mirrors (`pinecrest*`, `goldenmeadow*`) are dropped. Green Play template still lists `2embed.stream/embed/…` for sniff; that host is currently unreliable. Legacy top-level `2embed.cc` bounces to `.skin` and breaks sniff.
...
- Template embed hosts are kept on working player domains: VSEmbed (`vidsrc.sh/embed/…`), VidSrc (`video.moviepire.co/embed/…` behind the CAPTCHA-gated `vidsrc.win/watch/…` page), VidFast (`vidfast.vc`), VidRock (`vidrock.ru`), Vidzee (`player.vidzee.wtf/embed/…`), VidLove / 111Movies (`player.vidlove.cc/embed/…`), VidSrc.sbs (`vidsrc.sbs/embed/…`), AutoEmbed (`player.autoembed.co/embed/…` — not the outer `autoembed.co` shell), 2Embed (`2embed.stream/embed/…` per [2embed.online](https://www.2embed.online/)), VidAPI (`vidapi.xyz/embed/…`). Playback stamps each stream with that provider id and opens with that embed host as Referer — CDN hostname churn does not need a code patch; retarget the template/base in builtins when the **player** domain moves
