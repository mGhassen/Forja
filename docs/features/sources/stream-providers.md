...
- **VidLove** (`player.vidlove.cc`) and similar multi-server embeds: the sniffer rotates internal server chips (e.g. Neta / Gogo / Mafia / Fabric) when the default server is stuck loading, and attaches WebView cookies + the embed Referer when opening the CDN playlist
