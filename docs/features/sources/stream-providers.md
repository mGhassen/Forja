...
**Sources → Forja tab** (separate from green Play) runs bundled `extract(ctx)` HTTP plugins in QuickJS: Videasy, VidLink, VixSrc, DooFlix, YFlix, VidNest cipher, VidRock (`vidrock.net` passphrase AES, not the remote decrypt host), VidFast EncDec, VidSrc.sbs, Cinesrc, Hexa, VidCore, MoviesAPI vidora, HiAnime MegaPlay, KickAssAnime, 2DHive, MultiEmbed/2embed, template embeds, remaining EncDec/Yoruix/Anivexa HTML catalogs, and KissKh. File-host pages (Doodstream, VOE, Filemoon, MixDrop, FlixCloud, …) are internal **hops** — not chips; a plugin that lands on those URLs calls `ctx.hop(url)` before listing a row. HTTP miss still falls back to the built-in sniff extractor via `ctx.host`. Green Play / Tries sniff is unchanged. MyCima ships in the pack but stays off (Arabic is out of the product tabs).
...
- **VidRock** (`vidrock.net` API, passphrase AES locally): Forja tab encrypts `{tmdb}` / `{tmdb}_{s}_{e}` and GETs `/api/movie|tv/{encrypted}` — Astra playlists expand to quality rows. Green Play still sniffs `vidrock.ru/movie|tv/…` (Server List click) when the HTTP path misses.
