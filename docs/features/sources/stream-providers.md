...
|------|-----------|-----------|
| Direct extractors | Videasy, VSEmbed, Vidnest | Videasy: `api.speedracelight.com` mirrors (+ Servers-tab sniff fallback); VSEmbed: legacy HTML chain then `vsembed.su` WebView sniff; VidNest: `new.vidnest.fun` API (+ sniff fallback) |
...
- **Videasy** resolves via `db.speedracelight.com` + `api.speedracelight.com` mirrors (same as [player.videasy.to](https://player.videasy.to) Servers: Yoru→`cdn`, Cypher→`downloader2`, Breach→`m4uhd`, Neon→`vsrc`, Vyse / Killjoy / Fade / Omen / Raze — not the public embed docs on [videasy.to](https://www.videasy.to/docs)). Forja probes **every** mirror in bounded parallel and lists each responsive mirror under Sources as **one** stream row (e.g. `Yoru`); height ladders (`2160p` / `1080p` / …) move to the player **Quality** control. CDN-named singles (`Breach · playhq`) stay as their own row. Playback opens the first reachable stream and keeps the `player.videasy.to` Referer by provider identity. If all mirrors fail/CF-block, Forja sniffs `player.videasy.to` and rotates the Servers dropdown one by one, keeping every playlist that emits
...
- **VidSrc.sbs** (`vidsrc.sbs/embed/…`) is separate from **VSEmbed** (`vsembed.su`) and **VidSrc** (`vidsrc.win`). Its aggregator rejects iframe-wrapped playback, so Forja opens it as the headless WebView's top-level document before sniffing on desktop/mobile. On resolve, Forja reads the embed’s `CFG.servers` list (site order) and sniffs **every** nested mirror in bounded parallel (two at a time). Each nested player rotates its own Servers chips (e.g. Pro Multi internals) and keeps every playlist that emits — Sources lists them (e.g. `PRO Multi · …`, `Cinesrc`, `4K`). Playback opens the first reachable stream. Nested proxy responses are scanned for `.m3u8` in the body; Cinesrc-style `ice…/?m3u8=<token>` URLs are treated as playable HLS even without a `.m3u8` path. Policy lives under `shared/extractors/providers/vidsrcsbs/`
