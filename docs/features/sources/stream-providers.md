...
- **Manual** — on the loading screen, open the server list (layers icon next to **Cancel**) and tap a provider to check it now; or in the player pick a server / stream (or turn Auto server Off). Forja stays on that pick — **no** silent cross-provider/stream hop on fail. Switching providers mid-check starts the new one immediately (only one Source-panel spinner at a time); if the previous sniff had already found a playable URL, that hit is kept (not discarded).
