...
- **VidSrc.sbs** (`vidsrc.sbs/embed/…`) is a separate template embed from **Vidsrc** (`vsembed.su`). Its aggregator rejects iframe-wrapped playback, so Forja opens it as the headless WebView's top-level document before sniffing on desktop/mobile. Nested `1embed` proxy responses are scanned for `.m3u8` URLs in the response body (not only in the request URL). Policy lives on that provider’s `EmbedExtractProfile` — not global sniffer flags
- **VidLove** (`player.vidlove.cc`) and similar multi-server embeds: that provider’s profile rotates internal server chips (e.g. Neta / Gogo / Mafia / Fabric) when the default server is stuck loading, and attaches WebView cookies + the embed Referer when opening the CDN playlist
- Each HostRequired template provider has its own host sniff profile (`apps/forja/lib/shared/extractors/embed_extract_profile.dart`); the shared `StreamExtractor` is the WebView engine only
