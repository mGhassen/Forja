...
- The **score badge number** is the provider’s **running Σ** across titles you play (count **up or down**, never below **0** — fails at 0 stay 0; a later success counts up). The **+/− prefixes** are outcomes **for this film / TV episode / anime episode** only: server **±2**, stream **±2**. Server and stream are **linked**: extract OK + stream OK → **+2 +2**; extract OK + all streams dead → **+2 −2** (net 0); extract empty → **−2**. Score commits only when that check finishes — **Cancel**, mid-check, or extract-only (streams not finished) does **not** add server +2 alone. Asian drama is not scored. Σ nudges Auto order over time.
...
| Anime | Megaplay, VidNest, Miruro, AllAnime, … |
...
- **Videasy** resolves via `db.wingsdatabase.com` + `api.wingsdatabase.com` mirrors (same as [player.videasy.to](https://player.videasy.to) Servers: Yoru→`cdn`, Neon→`neon2`, Sage→`ym`, … — not the public embed docs on [videasy.to](https://www.videasy.to/docs)). Forja probes mirrors in parallel; the **first working stream opens after a short grace** (hung Neon/other servers do not block play). Playback keeps the `player.videasy.to` Referer by provider identity. If all mirrors fail/CF-block, Forja sniffs `player.videasy.to`
- **VSEmbed** uses `vsembed.su` embeds (`/embed/movie?tmdb=` · `/embed/tv?tmdb=&season=&episode=`); the inner CDN host (e.g. `cloudorchestranova.com`) is detected automatically — do not hardcode legacy `cloudnestra.com`. CloudStream playlists use tokenized `/pl/…/master.m3u8` URLs whose `page-N.html` segments reject `Referer`/`Origin` (browser no-referrer) — Forja opens them with User-Agent only. Cached or Continue Watching CloudStream links are dropped when the JWT expires or leaf segments 403, then re-resolved
...
- **VidFast** (`vidfast.vc/movie|tv/…`): loads as the headless WebView top-level page (no iframe wrapper). The site’s player is HLS.js / MSE — `video.src` is often a `blob:` while the real playlist is passed to `Hls.loadSource`. Forja hooks that call (and `#EXTM3U` response bodies) so the stream is captured for mpv
- **VidSrc.sbs** (`vidsrc.sbs/embed/…`) is separate from **VSEmbed** (`vsembed.su`) and **VidSrc** (`vidsrc.win`). Its aggregator rejects iframe-wrapped playback, so Forja opens it as the headless WebView's top-level document before sniffing on desktop/mobile. On resolve, Forja reads the embed’s `CFG.servers` list (site order) and sniffs nested mirrors in bounded parallel (two at a time). The **first working mirror opens after a short grace** — hung mirrors (e.g. nxsha) do not hold the spinner for a full sniff timeout. Streams found during that grace still appear in Sources. Nested proxy responses are still scanned for `.m3u8` in the response body. Policy lives under `shared/extractors/providers/vidsrcsbs/`
