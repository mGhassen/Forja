...
- The **score badge number** is the provider’s **global Σ** (sum of title totals for that server). The **+/− prefixes** are outcomes **for this film / TV episode / anime episode** only: server **±2**, stream **±2** (both ok → **+4** on this title; both fail → **−4** and Σ drops by 4). Asian drama is not scored. Those per-title totals feed Σ and nudge Auto order over time.
...
- **Videasy** resolves via `db.wingsdatabase.com` + `api.wingsdatabase.com` mirrors (same as [player.videasy.to](https://player.videasy.to) Servers: Yoru→`cdn`, Neon→`neon2`, Sage→`ym`, … — not the public embed docs on [videasy.to](https://www.videasy.to/docs)). Forja probes **Yoru/`cdn` first** (website default), then keeps collecting responsive sibling mirrors for up to eight seconds after the first hit. Every collected mirror appears as a named stream (Yoru, Neon, …); mirrors still hanging at the cutoff remain absent. If all mirrors fail/CF-block, Forja sniffs `player.videasy.to`
...
- **VidSrc.sbs** (`vidsrc.sbs/embed/…`) is separate from **VSEmbed** (`vsembed.su`) and **VidSrc** (`vidsrc.win`). Its aggregator rejects iframe-wrapped playback, so Forja opens it as the headless WebView's top-level document before sniffing on desktop/mobile. The embed exposes a **Server** dropdown (Star, PRO Multi, Cinesrc, Vlux, …) — the page defaults to Star (`1embed`), which often yields dead streams, so Forja opens the dropdown and prefers PRO Multi / Cinesrc / Vlux before accepting a playlist. Nested proxy responses are still scanned for `.m3u8` in the response body. Policy lives on that provider’s `EmbedExtractProfile` under `shared/extractors/providers/vidsrcsbs/`
