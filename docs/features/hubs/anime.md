...
The Anime tab is a full vertical for anime: hero carousel, mood chips, continue watching, curated rails (trending, airing, etc.), discover filters, search, and a dedicated player that probes available stream sources (Megaplay/Vidwish, VidNest, AllAnime, plus adult fallbacks when needed).
...
- Reorder anime sources under **Settings → Playback → Anime provider order** (same real provider names as the in-player source menu). Default try order starts with **Megaplay**, **AniKoto** (native site scrape), and **Vidwish**, then VidNest / AllAnime, then Miruro **AniKoto** / **AnimePahe** / **AllManga** / **AnimeDao**
- Megaplay prefers Anikoto `s-2` catalog embed ids (getSources works even when the Megaplay HTML page is a 410). `/stream/ani/…` remains a last-resort native URL but is not opened in the Web player. **AniKoto** races anikototv.to’s Ajax server list (Vidstream → MegaPlay, VidPlay → VidTube, …) into native HLS — same buttons as the site, without needing the WebView. Miruro pipes for **AniKoto**, **AnimePahe**, **AllManga**, and **AnimeDao** stay available (CF WebView). Each anime source has its own probe rule from Providers config (e.g. AnimePahe checks the playlist only; Megaplay still samples segments for PNG ads). Vidwish only appears when Anikoto matched an `s-2` id. Megaplay HLS CDNs (`mewstream`, `nekostream`, …) use a `megaplay.buzz` Referer for probe and play (including cached / reloaded streams). Segments that look like PNG “ads” but wrap real MPEG-TS (Megaplay anti-scraper) are unwrapped through the local HLS proxy and play; only true image-only poison is skipped. If the native player still cannot open streams, Forja opens **vidnest.fun** first (HiAnime / AnimePahe public players — same JS path as the browser), then Megaplay `/stream/s-2/…`, then **anikoto.cz**’s watch page (full page — the site blocks iframes). Picking **VidNest** manually and hitting a dead CDN/API opens that web player immediately. Anime hosts, path templates, KissKh mirrors, API bases, CDN Referer rules, and per-source probe profiles can be updated remotely (cloud `provider_runtime_config`) without an app update — offline builds keep the same built-in defaults
- **VidNest HiAnime** and **VidNest AnimePahe** resolve via `new.vidnest.fun` for native extract; the web fallback uses `vidnest.fun/anime/…` and `vidnest.fun/animepahe/…`. When Anikoto maps a different AniList id for the same title, that id is used for VidNest
