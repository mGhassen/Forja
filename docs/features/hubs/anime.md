...
- Browse hero and mood-based rails — titles follow **Settings → Addons → Anime → Anime title language** (**Romaji** by default; also under **Settings → Forja Packs** → Anime expand). The browse hero uses the **same cinematic carousel as Home** (global AniList trending — several slides). **This Season** is the current AniList season; **Recently Aired** is the last ~48h of TV/ONA episodes from the airing schedule. Catalog posters show rating, year, and episode count (no TV / ONA / FILM type badge). On **desktop / TV**, a top menu overlays the hero: **Search**, **Films**, **Series**, **Categories** (AniList genres). Films/Series/Categories **refetch** the hub from AniList (hero included); Films/Series narrow every rail (including Trending) to movies or series. **Upcoming** / **Airing** statuses show in hero meta; unreleased titles show **Coming soon** above **View details**. Hero and catalog posters have a glass **+** (My List / Simkl when connected). On **Android TV**, **OK** on Anime or **→** from the nav lands on hero **View details**; D-pad **↓** from the hero bleed walks Continue → Mood → This Season → the remaining rails (empty Continue is skipped); **↑** from the bleed row (Trending) lands on hero **View details**; remote **Back** from a rail returns to the **Anime** nav item.
- **Continue watching** in-progress series — hover play on desktop; D-pad graph skips empty rows on TV.
- **Search** — hub top-bar **Search** (desktop / TV / mobile). Two-column layout on desktop/TV: search field, last searches + recommendations on the left, results grid on the right. Plain title search via the Anime pack (`search` capability — no structured person/year DSL unless the pack declares `structured_search`). On **desktop**, focus the field and type; on **Android TV**, browse-focus until **OK** (or click) to type. Search runs on **Enter** / **OK** (or a recent / recommendation tap), not while typing. **Cmd+F** / **Ctrl+F** opens or focuses search on desktop.
...
- Choose **Anime title language** under **Settings → Addons → Anime** (**Romaji** / **English** / **Native**). Stream matching still tries romaji, then English, native, and synonyms.
- Choose **Episode list** (**Cards** or **Number chips**) under **Settings → Addons → Anime** or **Settings → Forja Packs** → expand Anime — same control as Home / Asian Drama / IPTV series details.
- Browse rails need the hub’s catalog API. If it fails, you see a short message and **Retry** (or a toast with **Refresh** when the page shell already loaded) — not raw HTTP codes. On **Android TV**, focus lands on **Refresh**; D-pad or Back dismisses the toast and returns to the control you had before. Re-tap the Anime tab to reload. After an AniList rate limit, the hub cools down and does **not** re-fire every rail in parallel.
