...
- Stream resolve probes mirror APIs in parallel first, then tries healthy hosts in your Settings order with a fresh (no HTTP cache) headless browser so KissKH can sign the stream key
