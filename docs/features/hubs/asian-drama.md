...
- While a stream resolves, the loading screen lists each KissKH mirror with CHECKING / UP / DOWN status (same style as Movies & TV). Tap a mirror to check it next. If kisskh marks the title **Upcoming** (countdown on the site, not published yet), Forja skips extract and explains that on the same screen — with the expected release date when kisskh provides one
...
- Stream resolve tries mirrors in your Settings order — each gets a fresh (no HTTP cache) headless browser session so KissKH can sign the stream key
