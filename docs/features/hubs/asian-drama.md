...
- While a stream resolves, the loading screen shows the drama artwork (same style as Movies & TV playback). If kisskh marks the title **Upcoming** (countdown on the site, not published yet), Forja skips extract and explains that on the same screen — with the expected release date when kisskh provides one
