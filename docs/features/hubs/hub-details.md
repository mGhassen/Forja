...
- **Pack rails** — Characters, Staff, Related, Trailers, recommendations — whatever the pack returns after enrich. Body order keeps Cast/Characters above More Like This. Host TMDB enrich **More Like This** (when the pack has no recommendations rail) opens [TMDB details](../movies-tv/tmdb-details.md), not another KissKH page. On **Android TV**, focused cast/crew circles get a white ring on the photo; focused body rows sit about **25%** down from the top (not flush to the edge); **↑** from Characters scrolls the page to the top and walks **episodes → seasons → Play → back**; films go **Play → back**.
