...
- **Play row** — green **Play** / **Resume** runs the hub play path (Megaplay / KissKH native / Forja Auto / webstreaming when enabled). White **Play** / **Resume** (link icon) opens **Sources** when TMDB matched and Direct torrent / Nuvio / Forja play sources are on — same panel chrome as [TMDB details](../movies-tv/tmdb-details.md). **Trailer** appears when TMDB enrich returned videos (same in-app trailer player as movie/TV details). Titles that have not premiered yet show **Coming soon** with the formatted premiere date instead of Play.
