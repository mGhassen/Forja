...
- **macOS** — compact floating player (rounded, shadowed, always-on-top, all Spaces). Drag freely (no corner snap). Hover for expand, Minimize, Close, ±15s, play/pause, and a scrubber (VOD). Opted out of Magnet/Rectangle so tiling apps do not steal the drag.
- **Windows** — same compact always-on-top window with free drag. Hover for expand, Minimize, Close, ±15s, play/pause, and a scrubber (VOD).
...
- **Desktop auto PiP** (opt-in) — enable **Settings → Playback → Auto picture-in-picture**. While video is playing, switching Mission Control Space (macOS) enters PiP automatically and the window follows you across Spaces. On Windows, switching virtual desktop also enters PiP (the compact window stays on the desktop you left — OS limit). Off by default. Exit with **Expand** (restore) or **Close** on hover.
...
- **Expand** restores the full player; **Minimize** sends the floating window to the dock/taskbar; **Close** exits playback
