...
- Change appearance (size, color, font, opacity, position) — green bordered tune icon next to Close in the Subtitles header on **MediaKit** and **ExoPlayer** (same chrome as Close; brightens on hover/focus). On **TV**, focus a slider and use **Left/Right** to adjust (no OK first). Color swatches show a brand-green circular focus ring. Sync **delay** applies on MediaKit only
