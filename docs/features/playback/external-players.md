...
- On macOS **IINA**: Forja runs header-protected streams through the local **hls-proxy**, then opens the proxy URL in IINA via **NSWorkspace** (App Sandbox blocks `Process.start` on `iina-cli` / `/usr/bin/open`)
