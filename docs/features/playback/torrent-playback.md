...
- Switch torrents / Stremio / Nuvio streams mid-playback via the link (magnet) button — opens the same right-side **Sources** panel as media details (**Torrents / Stremio / Nuvio**, Filters → Providers, search, source cards). **Filters** stays closed until you tap the tune control. Opens on **Torrents** when you are playing a local magnet (including magnets that came from Stremio/Torrentio), otherwise Torrents when available (else Nuvio, then Stremio); results cached ~30 minutes; the **selected** category chip has a **reload** icon for a fresh fetch; switching away cancels an in-flight load for the previous kind. Torrents and Stremio show every fetched row. Nuvio starts one selected scraper; **Load next provider** runs one more. Closing the panel stops any still-running category fetch. The link control also appears for **Stremio Direct / Nuvio** sessions even when the stream has no magnet (direct HTTP). Picking another **torrent** shows the loading card, resolves the magnet from scratch, and opens a fresh player. Stremio/Nuvio picks resolve and reload in-player. The layers **Source** server picker stays hidden during catalog Sources playback (magnet or Stremio Direct)
...
- Magnet metadata (DHT/trackers) can take up to about 90 seconds before Forja gives up — slower than opening the same link in a desktop torrent client is normal on a cold DHT, especially right after switching torrents
