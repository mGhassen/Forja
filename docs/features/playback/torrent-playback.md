...
When you play a torrent or magnet link, Forja uses the built-in torrent engine (librqbit) to stream pieces over HTTP on localhost. The engine waits until the start of the selected file has enough bytes for the player to probe the container, then opens playback — you may see a short “starting engine” wait on slow swarms. Multi-file torrents show a file picker so you choose which video to play. Works from [TMDB details](../movies-tv/tmdb-details.md) or debrid-cached torrents.

## How to open it

- Pick a torrent on [TMDB details](../movies-tv/tmdb-details.md); on desktop, hover a torrent row in Sources to copy its magnet
...
- Switch torrents / Stremio / Nuvio streams mid-playback via the link (magnet) button — **MediaKit and ExoPlayer** (including Android TV). Opens the same right-side **Sources** panel as TMDB details (**Forja / Torrents / Stremio / Nuvio** tabs, provider chips under the tabs, search, source cards). On **Android TV** the panel owns D-pad focus (same graph as details Sources); search needs **OK** before the keyboard; multi-file torrents use a D-pad file list. **Back** closes Filters if open, otherwise the panel — it does not leave the player. **Filters** stays closed until you tap the tune control. Opens on **Torrents** when you are playing a local magnet (including magnets that came from Stremio/Torrentio), otherwise Torrents when available (else Nuvio, then Forja, then Stremio); results cached ~1 hour (hover status ~15 minutes); the **selected** category tab shows an animated **…** while that kind is still loading (hover **…** for **X** to cancel), and a checking provider chip shows the same **…**, and a **reload** icon for a fresh fetch (on **Android TV**, **→** then **OK** on that icon); if Forja plugins are still installing, only the **Forja** tab spins — Torrents / Stremio / Nuvio stay usable; switching away cancels an in-flight load for the previous kind. New rows keep the list where you scrolled or hovered. Torrents and Stremio show every fetched row. Torrents has an **All** chip plus one chip per enabled built-in provider (and Jackett / Prowlarr when configured). Opening Torrents searches the first enabled indexer; **All** searches every enabled built-in chip (Jackett / Prowlarr stay separate); tap **All** again to clear them. Stremio fetches only the selected addon. Opening Nuvio with **All** selected (or any set of selected scraper chips) keeps up to 10 scrapers in flight (5 on TV); tap a scraper chip to add it to the in-flight search (that does **not** stop the others). Nuvio has an **All** chip (select every scraper; tap **All** again to deselect them all) or tap individual scrapers (tap again clears their rows). Forja **All** and selected plugin chips keep up to 10 plugins in flight (5 on TV); rows append as each finishes and tapping another plugin adds it to the search. Closing the panel stops any still-running category fetch. The link control also appears for **Stremio Direct / Nuvio** sessions even when the stream has no magnet (direct HTTP). Picking another **torrent** or Stremio/Torrentio **magnet** keeps the current video playing and shows the bottom-right loading card while the new magnet resolves; a fresh player opens only when the stream is ready. Direct HTTP Stremio/Nuvio picks still resolve and reload in-player. The layers **Source** server picker stays hidden during catalog Sources playback (magnet or Stremio Direct)
...
- On Android TV, Sources lists torrents without pairing; playing a magnet — including switching torrents in the player — needs a paired desktop in [LAN](../settings/lan.md) (dialog in front of Sources if unpaired or desktop offline; cancel stays on Sources and does not start P2P on the TV). **Back** closes that dialog and stays in the player. Direct HTTP Stremio/Nuvio still play on the TV. Phones can play magnets locally, or via the desktop when paired.
