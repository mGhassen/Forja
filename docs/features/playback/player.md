...
- **Episodes / Sources panels** — TV **Episodes** and torrent **Sources** (magnet button) open right-side panels. Player **Sources** and **Filters** use a translucent dark shell (no freeze-frame image). Anime / hub **Episodes** use frosted glass (BackdropFilter over the video, no screenshot). **Filters** docks to the left of Sources. TV Episodes may still use a captured frame for frost. Episodes include season picker, numbered range chips (**1 - 50**, **51 - 100**, …) when the list is long, episode thumbnails, runtime, synopsis, and resume progress. Close with the borderless **Close** control in the header
