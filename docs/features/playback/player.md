...
- Open **Source** (bottom right) — a right-side panel with server rows and stream lists underneath. Each server shows a score badge and status glyph: **...** (not loaded), spinner (loading), green dot (loaded), or red X (unavailable). Tap an unloaded server to fetch its streams — you can load several in parallel. Tap a stream to **check** it (status on the left; current playback is not interrupted). Hover and use the **play** arrow to switch. Failed checks lower that server’s live score; sort order refreshes the next time you open the panel. While **CHECKING SOURCES** runs, status glyphs match the status roulette; the panel stays open and tappable.
...
- Open **Episodes** from the bottom bar — a right-side panel lists the season’s episodes with thumbnail, title, air date (orange when not yet aired), runtime, and synopsis; switch season from the header when the show has multiple seasons; use numbered range chips beside the season control when there are more than 50 episodes. Picking another episode resolves the stream and reloads in-player (you stay in the player)
