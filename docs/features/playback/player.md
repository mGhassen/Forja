...
- Open **Source** (bottom right) — a right-side panel with server rows and stream lists underneath. Each server shows a **live score badge** (starts at domain tier, e.g. VixSrc **75**, then moves **up/down** as you check streams) and status glyph: **...** (not loaded), spinner (loading), green dot (loaded), or red X (unavailable). While **Playing now**, the badge stays at least the base tier. Tap an unloaded server to fetch its streams — you can load several in parallel. Tap a stream to **check** it (status on the left; current playback is not interrupted). Hover and use the **play** arrow to switch. While **CHECKING SOURCES** runs, status glyphs match the status roulette; the panel stays open and tappable.
