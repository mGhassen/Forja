...
- Open **Source** (bottom right) — a right-side panel with server rows and stream lists underneath. Each server shows an IPTV-style status glyph: **...** (not loaded), spinner (loading), green dot (loaded), play icon (playing now), or red X (unavailable). Tap an unloaded server to fetch its streams in the background — you can tap several at once to load in parallel. Loaded servers expand their stream list automatically; pick a stream to play. Servers keep your settings/scoring order within each group (playing first, then working, then the rest). While **CHECKING SOURCES** runs, status glyphs match the status roulette; the panel stays open and tappable.
