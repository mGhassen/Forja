...
- Open **Source** (bottom right) — a right-side panel with server rows and stream lists underneath. Each server shows a **score badge** for **this film, TV episode, or anime episode** (not global; Asian drama not scored). The score is the **net** of two verdicts, not a running tally: a **server** verdict (up **+2** when it extracts streams, **−2** when it fails to resolve) plus a **stream** verdict (**+2** when at least one stream plays/probes OK, **−2** when every stream is dead). So an up server whose streams are all dead nets **+2 − 2 = 0**, an up server with a working stream nets **+4**, and a server that never resolved floors at **0**. A **+/− prefix** shows the last change; re-checking the same title re-evaluates the verdicts instead of drifting. **CHECKING SOURCES** and any server that already has extracted streams score the server verdict **+2**; after playback starts, every cached server's streams are **auto-probed** in the background so the netted score reflects real stream health. Tap an unloaded server to fetch its streams. Tap a stream to **check** it (playback is not interrupted). Hover a live stream to reveal the **play** arrow and switch; failed streams never show play.
