...
- Open **Stream source** (bottom right, swap icon pill) to switch server and quality in one panel — servers appear as grouped sections with HLS/MP4 rows underneath; the active server is highlighted. The menu stays tappable while **CHECKING SOURCES** runs; picking another server cancels the in-flight check and starts the new one.
