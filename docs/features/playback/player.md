...
- Open **Source** (bottom right) — a right-side panel with server rows and stream lists underneath. Each server shows a **score badge** on a **settings base of 0** for **this film, TV episode, or anime episode** (not global; Asian drama not scored). Server and stream outcomes **add**: server fail **−2**, server up **+2**, stream ok **+2**, stream fail **−2**, all streams down **−2** — server+stream ok → **+4** total. A **+/− prefix** shows the last change. **CHECKING SOURCES** and any server that already has extracted streams score **+2** server; after playback starts, other cached streams are **auto-probed** in the background (status icon + stream **±2**). Tap an unloaded server to fetch its streams. Tap a stream to **check** it (playback is not interrupted). Hover and use the **play** arrow to switch.
