...
- Open **Stream source** (bottom right, swap icon pill) to switch server and quality in one panel — servers appear as grouped sections with HLS/MP4 rows underneath; the active server is highlighted
- When you leave the player and press **Play** again for the same movie or episode in the same session, Forja reopens the last working web stream directly. If you switched server or quality in the player, that selected source is remembered first instead of rechecking every provider.
