...
- **Episodes / Sources panels** — TV **Episodes** and torrent **Sources** (magnet button) open frosted right-side panels. In the player the frost is a blurred screenshot of the current frame (not live video blur — that freezes macOS). **Filters** from Sources docks as a second frosted panel to the left of Sources. Episodes include season picker, numbered range chips (**1 - 50**, **51 - 100**, …) when the list is long, episode thumbnails, runtime, synopsis, and resume progress. Close with the borderless **Close** control in the header
