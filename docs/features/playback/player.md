...
- **Bottom bar** — full-width seek bar, then play/pause, ±10s, volume, and **current / total** time on the left; **Servers**, **Sources**, episodes (TV), audio, subtitles, quality, settings, and fullscreen on the right. On desktop torrent playback, when **Settings → Torrent stats** is on, a transparent stats card sits bottom-right above the seek bar while controls are visible (download/upload speed, peers, download size/%, ETA). Seeders show as — unless indexer metadata is available later — librqbit does not expose a live seeder count.
...
- **Episodes panel** — TV **Episodes** opens a frosted right-side panel (blurred freeze-frame behind the panel — same glass look as media-details Sources) with season picker, numbered range chips (**1 - 50**, **51 - 100**, …) when the list is long, episode thumbnails, runtime, synopsis, and resume progress. Close the panel with the same borderless **Close** control in the header
