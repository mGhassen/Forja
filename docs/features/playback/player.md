...
- Open **Source** (bottom right) — a right-side panel with server rows and stream lists underneath. Each server shows a **domain score badge** (configured tier, e.g. VixSrc **75**) and status glyph: **...** (not loaded), spinner (loading), green dot (loaded), or red X (unavailable). The badge dims when reliability memory is low; it stays bright while that server is playing. Tap an unloaded server to fetch its streams — you can load several in parallel. Tap a stream to **check** it (status on the left; current playback is not interrupted). Hover and use the **play** arrow to switch. Failed checks affect sort order the next time you open the panel. While **CHECKING SOURCES** runs, status glyphs match the status roulette; the panel stays open and tappable.
