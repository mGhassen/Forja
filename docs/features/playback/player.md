...
- **Status roulette** — center-right slide-up indicator when checking sources, switching servers, or buffering (no bottom snackbars). Episode switches use a separate centered loading card instead
- **Episode switch card** — centered translucent card when picking another episode or using Next/Previous: episode label plus live status (`Loading episode info…`, `Checking sources…`, provider name, `Opening stream…`)
...
- **Floating menus** — audio, subtitles, quality, and settings open as compact popup panels anchored to the button you tapped (above or below, depending on space). Nested menus show a **Back** control (muted until hover or D-pad focus, then white) when you drill into a sub-list. Menus use a dark Lab-style chrome: bordered cards, outlined option chips, and **white fill / black text** for the selected option. Settings groups Auto selection, decode, speed, aspect, and loop into section cards with On/Off or value chips. Dismiss with the outlined square **Close** control in the menu header
- **Episodes / Sources / Source panels** — TV **Episodes**, torrent **Sources** (magnet button), and web **Source** (server picker) open right-side panels. Player **Episodes**, **Sources**, **Source**, and **Filters** use a translucent dark shell (no freeze-frame image; no BackdropFilter over live video). **Filters** docks to the left of Sources. **Source** lists servers with IPTV-style status glyphs (**...** / spinner / dot / play / failed) — tap a server to load its streams in parallel. The **Episodes** top bar has a brand-green **Season** pill (when the show has multiple seasons), a combined **Search** field + **Auto next** switch (skip-next icon), and a squared **Close** control. A range dropdown (**1 - 50**, **51 - 100**, …) appears below when the list is long. Episode rows show thumbnails, air date (orange when not yet aired), show backdrop with dim overlay on not-yet-aired thumbnails, runtime, synopsis, and resume progress. Not-yet-aired episodes stay visible but cannot be opened.

Seek bar uses Forja brand green for the played fill and thumb (including hover/drag) across the main player, trailer player, ExoPlayer chrome, IPTV VOD, music, and audiobook players. On desktop the main player also supports hover frame preview (timestamp fallback when preview is unavailable). **Source** is one flat bottom-right control showing the active server name; tap it for the grouped list. The player handles torrent streams, direct HLS/MP4 links, debrid URLs, Jellyfin, and hub content.
...
- Open **Settings** popup for auto selection, **Auto skip intro**, speed, aspect ratio, loop, hardware decode, and subtitle style — each in a bordered section card with On/Off or value chips (same Lab chrome as other floating menus). On desktop torrent sessions, toggle **Torrent stats** (off by default) to show live download/peer info above the seek bar while controls are visible
