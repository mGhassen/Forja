...
- **Floating menus** — source, audio, subtitles, quality, and settings open as compact popup panels anchored to the button you tapped (above or below, depending on space). The source panel is a flat grouped list: server name as a text header, stream rows underneath with a type column (HLS / MP4) — no cards or nested boxes. Dismiss with the borderless **Close** control (soft circular highlight on hover — no outline box)
...
- Open **Source** (bottom right) to switch server and quality — flat list with server name headers and stream rows underneath (HLS / MP4 type column). The current server and its playing stream are listed first. While **CHECKING SOURCES** runs, other servers show **Checking…** or **Unavailable** to match the status roulette; the menu stays tappable and picking another server cancels the in-flight check.
