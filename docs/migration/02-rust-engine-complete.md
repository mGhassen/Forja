...
- `packages/forja_rust` is a **thin FFI loader only** (facade + bindings + library_path)
...
- [ ] `libtorrent_flutter` removed from all pubspecs (`apps/forja`, `forja_streaming`, `forja_api`)
...
- [ ] Orphan engine copies deleted from `forja_rust/lib/src/` (if any remain)
...
|-----|---------|--------------|
| Mobile librqbit won't compile | B2 — `librqbit-dualstack-sockets` / `bind_device` on iOS | P2-10 |
...
| ID | Task | Detail |
|----|------|--------|
| P2-10 | Fix librqbit iOS/Android build | Root cause: `librqbit-dualstack-sockets 0.7.0` → `Socket::bind_device` unavailable on iOS |
| P2-11 | Probe + CI | `./scripts/try_build_mobile_torrent.sh ios` · keep informational CI job |
| P2-12 | Enable full features in mobile build | `scripts/build_rust_mobile.sh` · Android Gradle · iOS Xcode phase — drop `--no-default-features` for torrent |
...
Files: `crates/forja-torrent/`, `packages/forja_streaming/lib/src/torrent_stream_service.dart`

### Drop libtorrent / Rust-only torrent

| ID | Task | Files |
|----|------|-------|
| P2-20 | Remove `libtorrent_flutter` from pubspecs | `apps/forja/pubspec.yaml`, `packages/forja_streaming/pubspec.yaml`, `packages/forja_api/pubspec.yaml` |
...
| ID | Task | Files |
|----|------|-------|
| P2-30 | Webstreamr: remove Dart HTML parse fallbacks | 44 files in `packages/forja_webstreamr/` — keep `tryRust*` only, fail if backend null |
| P2-31 | Delete orphan engine copies | `packages/forja_rust/lib/src/dart_fallback/`, `lib/src/reference/` if present |
...
| ID | Task | Detail |
|----|------|--------|
| P2-40 | Port remaining scrapers to Rust | YTS, EZTV, EliteTorrent, etc. — `crates/forja-scrapers/` |
...
|------|----------------|--------------|
| Webstreamr page fetch + registry | `forja_webstreamr` fetcher, registry | HTTP orchestration |
| Scraper search HTTP | `forja_scrapers` base_scraper, aggregator | HTTP orchestration |
...
- [ ] `./scripts/build_rust_mobile.sh all` with full features (torrent + proxy)
- [ ] `./scripts/try_build_mobile_torrent.sh ios` passes (or upstream fix documented)
