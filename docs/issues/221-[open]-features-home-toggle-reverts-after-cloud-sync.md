...
4. Pack purge on device sometimes skipped `scheduleForjaSyncPush`.

**After:** Await Features writes; immediate navigation + Forja pack pushes; skip cloud nav apply only while a local Features edit is unsynced; web preserves empty Features; pack remove/purge updates cloud.
...
2. Empty cloud → web shows all toggles **off** (not all on).
3. Turn Home on in the app → stays on after tab switch / focus.
