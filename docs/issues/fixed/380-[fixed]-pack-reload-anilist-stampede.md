...
**Root fix:** pack script wipe flags `PluginRegistry` (`[HubReload] flagged …`) and drops in-flight layout so a load waiting on install cannot scrape and mark the tab fresh. Opening that hub logs `[HubReload] opened …` and is the only scrape. Pack **settings** (`forceNetwork: false`) still soft-reload while that hub is selected. Hide/show pushes `shellTabVisible` into chrome; gates refuse off-tab binds.
