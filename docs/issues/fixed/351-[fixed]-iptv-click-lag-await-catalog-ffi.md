...
Favorite-star lag ([349](349-[fixed]-iptv-favorite-star-lag.md)) was one control. Broader “every click feels dead” came from:
...
- T03 one-frame yield (symptom) superseded by T04
- **T04:** `EngineAsyncJob.iptvCatalog` → `spawn_blocking` + `catalog_db::handle_json`; Dart `hasShelf` / `page` / `replaceShelf` / `clearAll` await `runIptvCatalogJson`

### Related

- [349](349-[fixed]-iptv-favorite-star-lag.md) — channel/portal star optimism  
- [352](352-[fixed]-app-wide-list-click-lag.md) — pack index + panel rebuilds  
