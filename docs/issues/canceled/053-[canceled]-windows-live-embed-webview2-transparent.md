...
| | |
|--|--|
| **Progress** | **Canceled** — product path gone |
...
Windows Live Matches WebView2 embed path gone; workaround obsolete.
...
On Windows, opening a Live Matches stream pushed `_LiveMatchesEmbedPlayerScreen`. After the Jul 14–15 macOS embed rewrite (iframe wrapper + `supportMultipleWindows`, issues [046](046-[canceled]-streamed-live-embed-white-screen.md) / [049](049-[canceled]-live-embed-ad-hijack-crash.md)), Windows showed a **white transparent** surface; Escape still worked. Streams had worked on the prior direct `initialUrlRequest` path.

**Symptom (original):** White / see-through player after launching a working stream; Escape backs out.

**Symptom (post-`I53-T02`):** Direct top-level embed on Windows often left a blank/black player (Streamed needs catalog `document.referrer` + successful off-screen `window.open` — same root as [046](046-[canceled]-streamed-live-embed-white-screen.md)).
