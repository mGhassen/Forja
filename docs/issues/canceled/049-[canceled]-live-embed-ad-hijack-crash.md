...
Live Matches embed player removed; ad main-frame hijack path no longer exists.
...
**Root:** Third-party embed ads navigate / popup the **host WebView main frame**. Canceling those navigations under rebuild churn (`onLoadStart` → `setState`) correlated with native WKWebView teardown and a hard crash. Distinct from [046](046-[canceled]-streamed-live-embed-white-screen.md) (white screen / spinner), which remains the subframe-allow + iframe-wrapper track.
