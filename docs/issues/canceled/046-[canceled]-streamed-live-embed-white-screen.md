...
Live Matches embed WebView surface deleted; Forja Live is native-only (`no-embed-playback`).
...
**Related:** Windows-only see-through white surface (Escape still pops) is [053](053-[canceled]-windows-live-embed-webview2-transparent.md) — WebView2 `DefaultBackgroundColor` alpha bug, not this embed/ad stall track.
