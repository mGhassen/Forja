...
| 6 | I190-T06 | Play closes Sources with `cancelEngineJobs: false` — skip `EngineService.cancelPending` | ✅ |
...
| 4 | I190-A04 | Play mid All-walk — no cancel→`flutter_js` stampede (`start` without rust-js); no Lost connection | ⬜ |

---

## Summary

**Symptom:** Selecting multiple Forja chips (even 3) while videasy runs → `Lost connection to device` after a peer finishes. Under All, videasy times out empty though alone it returns streams in ~6s. **Also:** Play while All walk runs → peers flip from `(rust-js)` to bare `[engine] X start` + `[ep] drop` then SIGSEGV.
...
**Root fix:** [RFC-064](../rfc/064-[open]-rust-quickjs-engine-runtime.md) — QuickJS-in-Rust per extract on tokio + `scope_job_token` task-local cancel (NuvioMobile shape). Nuvio host scrapers unchanged. **Plus** never map cancel-null → `flutter_js`; Play must not `cancelPending` Engine extracts.
...
**Shipped for I190-T05 / T06:** `service.dart` `runPluginIsolated` gen gate; `details_screen_stremio.dart` gates `cancelPending` on `cancelEngineJobs`.
