...
|--:|----|-------------|--------|
| 1 | I190-T01 | RFC-064 Forja EngineJS crate + STREAMCRYPTO + fetch bridges | ✅ |
...
**Symptom:** Selecting multiple Forja chips (even 3) while videasy runs → `Lost connection to device` after a peer finishes. Under All, videasy times out empty though alone it returns streams in ~6s. **Also:** Play while All walk runs → peers flip from `(enginejs)` to bare `[engine] X start` + `[ep] drop` then SIGSEGV. Skipping `cancelPending` on Play avoided the stampede but left scrapes running in the background.
