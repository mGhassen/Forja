...
| Layer | Status | Notes |
|-------|--------|-------|
| **Symptom** — UI thread blocks on addon HTTP | **workaround** | `runStremioHttpGet` → worker isolate |
| **Root** — blocking HTTP in `stremio-core` | **open** | [015](015-[open]-rust-blocking-http-engine-debt.md) |
...
Isolate offload — stops UI freeze but **does not fix** blocking HTTP in `crates/stremio-core`. Root fix: [015](015-[open]-rust-blocking-http-engine-debt.md).
...
Track in [015](015-[open]-rust-blocking-http-engine-debt.md): async HTTP in `crates/stremio-core`.
