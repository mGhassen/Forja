...
4. **Deferred (optional):** async job/poll FFI to reduce isolate spawn churn — not required for this issue.
...
- [ ] Profile: long resolve no longer spawns new isolate per call — **deferred** (job API not adopted; isolate-per-call remains)
...
| `packages/rust/lib/src/engine.dart`, `facade.dart` | `engineCancelPending`, `Engine.cancelPendingResolve` |
