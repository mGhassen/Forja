...
| | |
|--|--|
| **Progress** | **Canceled** — MutStreams removed from product |
| **Current slice** | Canceled — Servers no longer list MutStreams; Forja Live is native-only |
...
MutStreams was removed from Live Matches Servers (changelog ~1.4.128). Host has no MutStreams / `mut.st` path. Embed playback for Forja Live is forbidden (`no-embed-playback`). Historical rows below stay frozen.
