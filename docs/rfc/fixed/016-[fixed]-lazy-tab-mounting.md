...
| | |
|--|--|
| **Progress** | **Complete** · **5 / 5** acceptance (mount) |
| **Current slice** | Complete — eviction/stale in RFC-024 |
| **Backlog** | — (mount); eviction/stale → [RFC-024](024-[fixed]-tab-cache-eviction-stale.md) · [0.8.2](../backlog/done/0.8.2-[done].md) |
...
Tab cache **eviction** and **stale refresh** are specified in [RFC-024](024-[fixed]-tab-cache-eviction-stale.md) — do not conflate with this RFC.
...
RFC-011, RFC-017 (defer engines — complementary), RFC-001 (shell owns nav), [RFC-024](024-[fixed]-tab-cache-eviction-stale.md) (eviction + stale), [0.8.2 backlog](../backlog/done/0.8.2-[done].md)
