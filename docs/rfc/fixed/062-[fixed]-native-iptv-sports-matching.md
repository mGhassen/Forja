...
| | |
|--|--|
| **Progress** | **Complete** · **6 / 6** · Xtream/ATV/Stalker ✅ · **0 / 3** M3U ⏭️ |
...
| 14 | R62-A19 | Forja Sports schedule = same enabled Catalog JS as Forja Live / All (lazy chips + Settings toggles) | ✅ |
...
Host Flutter matching policy (leagues maps, broadcast soft-match, `IptvPortalSportsConfig` / `ensureArmed`, host `sportMatchGame` builder) is **superseded by [RFC-096](096-[fixed]-iptv-channel-search-only.md)**. Engine `sport_match_streams` ranking and EPG acceptance rows above stay as shipped. Do not rewrite frozen ✅ rows here.
