...
CI / release: repo secrets `TMDB_API_KEY`, `TMDB_READ_ACCESS_TOKEN`, `WYZIE_API_KEY`, `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY` (falls back to legacy `SUPABASE_ANON_KEY`), **`FORJA_WEB_URL`**, and optional **`TURNSTILE_SITE_KEY`** are injected by `.github/workflows/{build,release}.yml`. Flutter builds receive Supabase + portal URL (+ captcha site key) through `--dart-define`; use the hosted project values in GitHub, never the local `127.0.0.1` URL.
