...
**Webstreamr:** Lives in `crates/webstreamr` because C2 scrape + C7 proxy + playback pipeline = **engine** — same rule as TMDB (C1). UI freeze **workaround**: isolate offload (P2-91). Root fix: [issue 015](issues/015-[open]-rust-blocking-http-engine-debt.md). See [issue 001](issues/001-[workaround]-webstreamr-blocks-ui.md).
...
- **Forbidden:** sync FFI on UI thread for calls expected to exceed ~50ms — use `Isolate.run` ([P2-91](issues/001-[workaround]-webstreamr-blocks-ui.md)); workaround until [015](issues/015-[open]-rust-blocking-http-engine-debt.md)
