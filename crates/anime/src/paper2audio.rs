...
pub fn auth_token() -> Result<String, String> {
    ...
    let resp = http::fetch_with_retries("POST", &url, &headers, Some(&body), None, false, 30, 0)?;
    if resp.status >= 400 {
        return Err(format!("auth failed: {} {}", resp.status, resp.body));
    }
...
pub fn upload(file_name: &str, voice_id: &str, epub_bytes: &[u8]) -> Result<P2aUploadResponse, String> {
    ...
    let resp = http::fetch_with_retries(
        "POST",
        &upload_url,
        &headers,
        None,
    ...
    if resp.status >= 400 {
        return Err(format!("upload failed: {} {}", resp.status, resp.body));
    }
    let data: Value = serde_json::from_str(&resp.body).map_err(|e| e.to_string())?;
    let run_id = data
        .get("runId")
...
pub fn check_status(run_id: &str) -> Result<P2aStatusResponse, String> {
    let body = json!({ "runIds": [run_id] }).to_string();
    let mut headers = std::collections::HashMap::new();
    headers.insert("Content-Type".to_string(), "application/json".to_string());
    let resp = http::fetch_with_retries(
        "POST",
