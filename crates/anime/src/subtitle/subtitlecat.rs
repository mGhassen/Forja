...
fn search(query: &str) -> Result<Vec<SearchHit>, String> {
    ...
    for cap in SEARCH_RE.captures_iter(&resp.body) {
...
fn fetch_detail(detail_url: &str) -> Result<DetailPage, String> {
    let resp = http::fetch_with_retries("GET", detail_url, &HDRS, None, None, false, 15, 0)?;
    if resp.status != 200 {
        return Err(format!("detail {}", resp.status));
    }
    Ok(parse_detail_page(&resp.body))
}
