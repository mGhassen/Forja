...
static NOT_FOUND: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"No such file|File Not Found").unwrap());
...
pub fn extract_from_html(
    ...
    let title = Html::parse_document(download_html)
    ...
    let (height, bytes) = META_RE
        ...
        .unwrap_or((None, None));
    let _ = html;
    let url = build_stream_url(mfp, "LuluStream", page_url)?;
