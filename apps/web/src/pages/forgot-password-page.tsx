...
const forgotPasswordRoute = getRouteApi('/_auth/forgot-password')

function ForgotPasswordForm() {
  const navigate = useNavigate()
  const { sent: sentFromSearch } = forgotPasswordRoute.useSearch()
  const { requestPasswordReset, configured } = useAuth()
  const [email, setEmail] = useState(sentFromSearch ?? '')
              ...
              <CardContent>
                <ol className="space-y-3 rounded-xl border border-[rgba(237,230,218,0.12)] bg-black/20 px-4 py-4 text-sm leading-relaxed text-[rgba(237,230,218,0.55)]">
                  ...
                  <li>
                    ...
                    Click{' '}
                    <span className="text-[#EDE6DA]">Choose new password</span>
                  </li>
                  <li>
                    <span className="font-mono-ui text-[10px] uppercase tracking-[0.14em] text-forja-green">
                      3
              ...
              <form onSubmit={onSubmit} className="space-y-5">
                {!configured ? (
                  <p className="text-sm leading-relaxed text-[rgba(237,230,218,0.55)]">
                    Password reset is not open yet. Download Forja - you can
