-- Optimize IPTV schema:
-- 1) scrape_posts: drop unused Reddit text / funnel columns (post_id only)
-- 2) drop scrape deep_refs table (unused by ops UI)
-- 3) fold catalog candidates into iptv_portals via catalog_pool flag

-- ---------------------------------------------------------------------------
-- scrape_posts: id-only
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- iptv_portals: catalog pool fields (was iptv_catalog_candidates)
-- ---------------------------------------------------------------------------
...
-- Passwords on candidates are already ciphertext — skip re-encrypt on copy.
alter table public.iptv_portals disable trigger iptv_portals_encrypt_password;

-- Migrate candidates → portals (same url/username unique key).
...
-- Admin decrypt: same RPC name, now reads iptv_portals
...
-- Drop old candidates table (+ encrypt trigger goes with it)
