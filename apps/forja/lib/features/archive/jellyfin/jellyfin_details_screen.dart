...
import 'package:forja/shared/widgets/shell/hover_scale.dart';
...
class _JellyfinDetailsScreenState extends State<JellyfinDetailsScreen> {
  ...
  final FocusNode _heroPlayFocus = FocusNode(debugLabel: 'jellyfin-details-play');
  ...
  Widget _buildPlayButtons(JellyfinItem item) {
    ...
    final tv = _tvNav;
    final heroCount = hasProgress ? 2 : 1;
        ...
        builder: (context) {
          ...
          return Column(
            children: List.generate(
              _episodes.length,
              (i) => _buildEpisodeCard(_episodes[i], index: i),
            ),
          );
        },
      ),
  ...
  Widget _buildEpisodeCard(JellyfinItem ep, {required int index}) {
                  ...
                  child: Stack(
                    ...
                    children: [
                      if (thumbUrl != null)
                        ...
                        const Center(
                          child: Icon(Icons.movie_rounded, color: Colors.white12, size: 28),
                        ),
                      Center(
                        child: Container(
                          padding: const EdgeInsets.all(8),
    ...
    final tv = _tvNav;
    if (!tv) {
      return _HoverCard(onTap: () => _playItem(ep), child: card);
    }
  ...
  Widget _buildSimilarSection() {
    final tv = _tvNav;
    final similarOrder = _details?.type == 'Series' ? 2 : 1;

    if (tv) {
      shellTvRegisterRow(
        tabId: MediaDetailsTv.tabId,
        rowId: 'similar',
        sortOrder: similarOrder,
                      ...
                      Expanded(
                        child: Container(
                          ...
                          child: posterUrl != null
                              ...
                              : const Center(
                                  child: Icon(Icons.movie_rounded, color: Colors.white12, size: 32),
                                ),
                        ),
                      ),
                      const SizedBox(height: 8),
