...
abstract final class IptvPortalSportsMatchService {
  ...
  static Future<LiveBroadcastHints> broadcastHintsFor(MatchEvent match) =>
      _broadcastHintsForMatch(match);

  static void invalidateBroadcastCaches() => invalidateLiveBroadcastCaches();

  /// Prefer pack-supplied [MatchEvent.sportMatchGame]; else derive from title.
  static Map<String, dynamic> sportMatchGameForResolve(MatchEvent match) {
    if (match.sportMatchGame != null) {
      return Map<String, dynamic>.from(match.sportMatchGame!);
    }
    return _sportMatchGamePayloadFromMatch(match);
  }

  static MatchEvent copyMatch(
    MatchEvent m, {
...
// IPTV sports matching uses pack-supplied sportMatchGame / broadcast catalogs.
// Host ESPN browse merge was removed (RFC-073) — ESPN schedule is catalog-espn.
...
bool _teamPairsMatch(String? home, String? away, Map<String, dynamic> other) {
  final epair = matchTeamPairKey(
    (other['homeTeam'] ?? '').toString(),
    (other['awayTeam'] ?? '').toString(),
  );
  final pair = matchTeamPairKey(home, away);
  if (pair != null && epair != null && pair == epair) return true;

  final nickPair = matchTeamPairKey(
    (other['homeNick'] ?? '').toString(),
    (other['awayNick'] ?? '').toString(),
  );
...
bool _broadcastTeamSideMatches(String a, String b) {
  final left = matchTextKey(a);
  final right = matchTextKey(b);
...
bool _broadcastTitleTokenOverlap(String leftTitle, String rightTitle) {
  final left = matchTextKey(leftTitle);
  final right = matchTextKey(rightTitle);
...
bool _sameCatalogEventAsBroadcast({
  ...
  final broadcastTitleKey = matchTextKey(broadcastTitle);
  final catalogTitle = matchTextKey(title);
...
/// Opaque broadcast channel names keyed by pack source id (`broadcastBySource`).
class LiveBroadcastHints {
  const LiveBroadcastHints({this.bySource = const {}});

  final Map<String, List<String>> bySource;

  bool get isEmpty =>
      bySource.isEmpty || bySource.values.every((v) => v.isEmpty);

  List<String> get allChannels => [
        for (final list in bySource.values) ...list,
      ];
}
...
List<String> _broadcastSearchTokens({
  ...
  return {
    ...
    ...matchTextKey(title).split(' ').where((t) => t.length >= 4),
  }.toList();
}
...
void _logBroadcastNearMisses({
  ...
  for (final row in rows) {
    ...
    final hay = matchTextKey(gameTitle);
...
Future<List<String>> _broadcastOnDemandLookup({
  ...
  try {
    _logBroadcastHints(
      '${plugin.id} on-demand broadcast lookup home="$home" away="$away"',
    );
    final rows = await EngineService.instance.runLiveFeed(
...
LiveBroadcastHints _liveBroadcastHintsFromGame(Map<String, dynamic>? game) {
  if (game == null) return const LiveBroadcastHints();
  final bySource = _broadcastBySourceMap(game);
  if (bySource.isEmpty) return const LiveBroadcastHints();
  return LiveBroadcastHints(
    bySource: {
...
Future<LiveBroadcastHints> _broadcastHintsForMatch(MatchEvent match) async {
  ...
  final bySource = <String, List<String>>{
    for (final e in fromGame.bySource.entries)
      e.key: List<String>.from(e.value),
  };

  if (bySource.isNotEmpty &&
      bySource.values.every((v) => v.isNotEmpty) &&
      bySource.length >= 2) {
    _logBroadcastHints(
      'using enriched game payload sources=${bySource.keys.join(",")} '
      'channels=${bySource.values.fold<int>(0, (n, v) => n + v.length)}',
    );
    return fromGame;
  }
  ...
  if (bySource.isNotEmpty) {
    _logBroadcastHints(
      'partial game payload sources=${bySource.keys.join(",")} — '
      'filling gaps from broadcast catalogs',
    );
  }
  ...
  if (plugins.isEmpty) {
    if (bySource.isEmpty) {
      _logBroadcastHints(
        'no broadcast plugins enabled — turn on broadcast catalogs '
        'in Settings → Forja Sports → Catalog',
      );
    }
    return LiveBroadcastHints(bySource: bySource);
  }
  ...
  for (final plugin in plugins) {
    final sourceKey = LiveSportCapabilities.normalizePluginId(plugin.id);
    if ((bySource[sourceKey] ?? const []).isNotEmpty) continue;
    ...
    bySource[sourceKey] = catalogChannels.toSet().toList();
    if (matchedRows == 0) {
      _logBroadcastHints(
        '${plugin.id} no fixture match — source section empty',
      );
    }

    if ((bySource[sourceKey] ?? const []).isEmpty) {
      final onDemand = await _broadcastOnDemandLookup(
        plugin: plugin,
        home: home,
        away: away,
      ...
      if (onDemand.isNotEmpty) {
        bySource[sourceKey] = onDemand.toSet().toList();
  ...
  final hints = LiveBroadcastHints(bySource: {
    for (final e in bySource.entries)
      if (e.value.isNotEmpty) e.key: e.value,
  });
  if (hints.isEmpty) {
    _logBroadcastHints(
      'result empty — fixture not listed with TV data on broadcast catalogs',
    );
  } else {
    _logBroadcastHints(
      'result sources=${hints.bySource.keys.join(",")} '
      'channels=${hints.allChannels.length} '
      'sample=${hints.allChannels.take(4).join(", ")}',
    );
  }
  return hints;
}
...
String _iptvSportsStreamsCacheKey({
  ...
  return '$portalKey|${cats.join(',')}|${matchEventViewerKey(match)}';
}
...
Map<String, dynamic> _sportMatchGameAlignedWithCard(MatchEvent match) {
