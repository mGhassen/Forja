...
class _LanSettingsSectionState extends State<LanSettingsSection> {
    ...
    if (enabled) {
      ...
      if (!ok && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to start LAN server')),
        );
        return;
      }
    } else {
  ...
  Future<void> _pairWith({String? host, int? port}) async {
    ...
    if (h.isEmpty || p <= 0 || code.length < 6) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Enter server address and 6-digit code')),
      );
      return;
    }
    ...
    if (token == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Pairing failed — check code and address')),
      );
      return;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Paired with desktop server')),
    );
