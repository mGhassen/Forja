...
String? _yearFromMeta(MetaItem? meta) {
...
SourcesRequestContext buildSourcesRequestContext({
  ...
  final hasCatalog = meta != null || open != null;
  ...
  final title = movie.title.trim().isNotEmpty
      ? movie.title.trim()
      : (meta?.name.trim() ?? '');
  final year = _yearFromMovie(movie) ?? _yearFromMeta(meta);
  ...
  final nuvioType = _nuvioMediaType(movie: movie, open: open, meta: meta);
