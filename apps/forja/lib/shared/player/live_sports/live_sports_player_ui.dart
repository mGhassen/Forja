...
mixin _LiveSportsPlayerUi on ConsumerState<LiveSportsPlayerScreen> {
    ...
    return Actions(
      ...
      child: PopScope(
        ...
        child: Stack(
          fit: StackFit.expand,
          children: [
            // Mini: Scaffold/MouseRegion are opaque hit targets — ignore them so
            // shell/rails receive clicks; corner slot below stays interactive.
            IgnorePointer(
              ignoring: mini,
              child: Scaffold(
            ...
            child: Stack(
              ...
              child: Stack(
                ...
                children: [
                  ...
                  // Full-player video only — mini mounts the same GlobalKey slot
                  // outside [IgnorePointer] so the corner stays hittable.
                  if (!mini)
                  Positioned(
  ...
  /// MediaKit / Exo surface — [GlobalKey] keeps the texture across full↔mini;
  /// [ValueKey] epoch remounts after engine recreate.
  Widget _buildIptvVideoSurface() {
    if (!_s._playerReady) return _playerSwitchingSurface();
    if (_s._exoBackend) {
      return KeyedSubtree(
        key: _s._videoViewKey,
        child: ExoPlayerView(
          key: ValueKey('iptv-exo-${_s._videoEpoch}'),
          viewId: _s._exoViewId!,
          // IPTV Exo always TextureView on ATV: physical SurfaceView + hybrid
          // composition went audio-only black (even cold-open) and the
          // composition-dead surface still fires renderedFirstFrame, so the
    ...
    PlayerAppMenu.show(
      ...
      onSelect: ({builtInEngine, externalPlayer}) async {
        ...
        final ok = await ExternalPlayerService.launch(
          url: url,
          title: _s._title,
          headers: const {'User-Agent': _LiveSportsPlayerScreenState._ua},
          context: context,
                ...
                : FutureBuilder<List<EpgEntry>>(
                    ...
                    builder: (context, snap) {
                      ...
                      return _liveProgressTimeLabel(
                        _LiveSportsPlayerScreenState._fmtDur(safe),
                        compact,
                      );
                    },
                  ),
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            ...
            child: Text(
              '${_LiveSportsPlayerScreenState._fmtDur(pos)} / --:--',
              textAlign: TextAlign.right,
              style: GoogleFonts.spaceMono(
                color: Colors.white,
                fontSize: ShellPaintScope.usesTvDensityOf(context)
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            ...
            child: Text(
              '${_LiveSportsPlayerScreenState._fmtDur(shownPos)} / '
              '${_LiveSportsPlayerScreenState._fmtDur(_s._duration)}',
              textAlign: TextAlign.right,
              style: GoogleFonts.spaceMono(
                color: Colors.white,
                fontSize: ShellPaintScope.usesTvDensityOf(context)
...
class _IptvSportsSourcePickerListState extends State<_IptvSportsSourcePickerList> {
  final _healthProbe = LazyUrlHealthProbe();
...
bool iptvUseLiveSportsSourcePicker(LiveSportsPlayerScreen screen) {
