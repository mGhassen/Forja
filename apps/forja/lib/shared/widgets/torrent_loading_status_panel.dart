...
/// Compact stat chips for in-player source switches ([PlayerEpisodeLoadingCard]).
class TorrentLoadingStatusCompact extends StatelessWidget {
  const TorrentLoadingStatusCompact({super.key, required this.status});
  ...
  Widget build(BuildContext context) {
    ...
    final peerText = peers != null
        ? (seen != null && seen > peers && seen > 0
            ? '$peers / $seen peers'
            : '$peers peers')
        : null;
    return Wrap(
      ...
      runSpacing: 6,
      children: [
        if (peerText != null) _chip(peerText),
        if (status.speedLabel != null) _chip(status.speedLabel!, accent: true),
        if (status.bufferLabel != null)
          _chip('${status.bufferLabel} buf', accent: true)
        else if (peers != null && peers > 0)
          _chip('buffering…'),
      ],
    );
  }
  ...
  Widget _chip(String text, {bool accent = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white.withValues(alpha: accent ? 0.12 : 0.08),
