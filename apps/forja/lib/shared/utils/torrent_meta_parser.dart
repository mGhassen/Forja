...
/// Parses torrent/stream title strings into quality, language, codec, and size tags for Sources badges and filters.
class TorrentMetaParser {
  ...
  static TorrentMetaParser parse(String name) {
    final n = name.toUpperCase();
    return TorrentMetaParser(
...
Set<String> collectSizeRanges(Iterable<double> sizesInBytes) {
  ...
  for (final bytes in sizesInBytes) {
    final range = TorrentMetaParser.sizeRangeForBytes(bytes);
