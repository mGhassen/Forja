...
import 'package:forja/shared/engine/portals/gate/iptv_forja_sports_gate.dart';
import 'package:forja/shared/engine/runtime/meta/plugin_actions.dart';
import 'package:forja/shared/engine/runtime/nav/plugin_nav.dart';
import 'package:forja/shared/host/layout/chrome/kit_portals_chip.dart';
...
/// Pack-driven `kit.topBar` `action: portals` — inventory via pack `listPortals`.
///
/// No product controller. Host only runs pack actions + paints foundation panel.
abstract final class PortalsChromeHooks {
  PortalsChromeHooks._();

  /// Pack `kit.list` source ids that hoist the portals overlay body.
  ...
  static Widget? _buildPortalsAction(
    ...
    // Portals-owning hub always shows the chip. Other tabs (Live Sports)
    // respect the Forja Sports pack toggle.
    final tabPlugin = PluginNavRegistry.pluginIdForTabSync(tabId) ?? '';
    final portalsPlugin =
        ref.watch(portalsInventoryProvider).asData?.value.pluginId ?? '';
    final isPortalsOwner =
        portalsPlugin.isNotEmpty && tabPlugin == portalsPlugin;
    if (!isPortalsOwner) {
