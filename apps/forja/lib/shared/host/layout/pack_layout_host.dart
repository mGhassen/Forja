...
export 'package:forja/shared/host/layout/pack_layout_host_wire.dart'
    show PluginKitTopBar;
...
class PackLayoutHost extends StatefulWidget {
  ...
  @override
  State<PackLayoutHost> createState() => _PackLayoutHostState();
}

class _PackLayoutHostState extends State<PackLayoutHost>
    with AutomaticKeepAliveClientMixin, ShellTabRefresh<PackLayoutHost> {
  ...
  void initState() {
    ...
    // never recovers when EngineCache warms. Wait for splash dismiss (prefetch
  ...
  /// Hub EngineCache wipe for this plugin (install / script edit / remove).
  ...
  @override
  void didUpdateWidget(covariant PackLayoutHost oldWidget) {
  ...
  Future<void> _loadLayout({bool forceRefresh = false}) async {
    ...
    if (!envelope.ok) {
      ...
      debugPrint(
        '[PackLayoutHost] ${widget.pluginId} layout failed: '
    ...
    if (invalid != null) {
      debugPrint(
        '[PackLayoutHost] ${widget.pluginId} layout invalid: $invalid',
      );
      setState(() {
        _loading = false;
  ...
  /// allows it. Product filters belong in those hooks — not here (RFC-109).
  ...
  Widget _buildCatalogBody(BuildContext context) {
    ...
    return CatalogShell(
      ...
      wrapBody: (b) => LayoutScope(
        ...
        focusEdge: (rowId, {last = false}) =>
            kitFocusEdge(_pageKey, rowId, last: last),
        child: TvFocusGraph(tabId: _pageKey, child: b),
      ),
    );
  }
}
...
/// Resolves [tabId] → installed hub [pluginId] before building [PackLayoutHost].
///
/// Prefer [PackLayoutHost.loader] / [PackLayoutHostLoader] for new mounts
/// (RFC-109). Kept for transitional call sites.
class PackLayoutHostLoader extends StatefulWidget {
  const PackLayoutHostLoader({super.key, required this.tabId});

  final String tabId;

  @override
  State<PackLayoutHostLoader> createState() => _PackLayoutHostLoaderState();
}

class _PackLayoutHostLoaderState extends State<PackLayoutHostLoader> {
  String? _pluginId;
  ...
  Widget build(BuildContext context) {
    ...
    if (pluginId != null) {
      // Avoid importing pack_layout_host here (cycle). Callers should use
      // PackLayoutHostLoader; PackLayoutHost remains the layout engine for Wave 1.
