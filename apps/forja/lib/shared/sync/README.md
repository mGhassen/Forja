...
Supabase Auth + `user_settings` / `profile_settings` blob sync. See [RFC-006](../../../docs/rfc/006-[partial]-supabase-sync.md) and [RFC-034](../../../docs/rfc/034-[partial]-web-portal-landing.md).
...
Import via `package:forja/shared/sync/sync.dart` unless a deep import is required.

## Dart-defines

- `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY` (same project as `apps/web`)
...
Browser: `DesktopBrowserAuth` opens `{FORJA_WEB_URL}/login?desktop_callback=…`;
