...
/// Opaque tab for [CatalogKitSourcesPanel].
@immutable
class CatalogKitSourcesTab {
  const CatalogKitSourcesTab({
    required this.id,
    required this.label,
  });
...
/// Opaque row for [CatalogKitSourcesPanel].
@immutable
class CatalogKitSourcesRow {
  const CatalogKitSourcesRow({
    required this.id,
    required this.title,
    this.subtitle,
    this.badges = const [],
...
class CatalogKitSourcesPanel extends StatefulWidget {
  const CatalogKitSourcesPanel({
    super.key,
    required this.title,
    required this.tabs,
  ...
  final String title;
  final String? subtitle;
  final List<CatalogKitSourcesTab> tabs;
  final String? initialTabId;

  /// Load rows for a tab id. Called on first select and on reload.
  final Future<List<CatalogKitSourcesRow>> Function(String tabId) loadTab;

  final Future<void> Function(CatalogKitSourcesRow row) onPlayRow;
  ...
  @override
  State<CatalogKitSourcesPanel> createState() => _CatalogKitSourcesPanelState();
}

class _CatalogKitSourcesPanelState extends State<CatalogKitSourcesPanel> {
  late String _tabId;
  final Map<String, List<CatalogKitSourcesRow>> _rowsByTab = {};
  ...
  @override
  void didUpdateWidget(CatalogKitSourcesPanel oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        _header(context),
        if (widget.tabs.length > 1) _tabs(context),
  ...
  Widget _body(
    ...
    required List<CatalogKitSourcesRow> rows,
  }) {
