...
class KitDetailsHero extends StatelessWidget {
  const KitDetailsHero({
    super.key,
  ...
  /// When set, right column uses the same [FactsPanel] as Home details.
...
class KitDetailsHeroSurface extends StatelessWidget {
  const KitDetailsHeroSurface({
    super.key,
    required this.backdropUrl,
    this.backdropUrls = const [],
    this.height,
...
class _KitHeroLayout extends StatelessWidget {
  const _KitHeroLayout({
    required this.title,
    this.subtitle,
  ...
  Widget build(BuildContext context) {
    ...
    if (useFullWidthList) {
      ...
      var hasFacts = false;
      if (rich != null) {
        final panel = FactsPanel.fromFields(
        ...
        final panel = FactsPanel(
          ...
          valueMaxLines:
              ShellScope.inputPolicyOf(context).useFocusableMoodChips ? 2 : 1,
        );
        hasFacts = panel.hasContent;
        factsChild = hasFacts ? panel : null;
      }
...
class _KitHeroMainColumn extends StatelessWidget {
  const _KitHeroMainColumn({
    required this.movie,
    this.logoUrl,
    required this.title,
    this.subtitle,
  ...
  Widget _titleWidget(BuildContext context, double slotHeight) {
    return HeroTitle(
      title: title,
      logoUrl: logoUrl,
      slotHeight: slotHeight,
      tvDensity: ShellScope.metricsOf(context).usesTvDensity,
      plainTitle: ShellScope.inputPolicyOf(context).useFocusableMoodChips,
          ...
          SizedBox(
            ...
            child: Align(
              alignment: Alignment.bottomLeft,
                  child: _titleWidget(context, titleHeight),
            ),
          )
        else
          _titleWidget(context, _defaultTitleHeight),
      if (showSubtitle) ...[
...
class KitHeroMetaLine extends StatelessWidget {
  const KitHeroMetaLine({
    super.key,
    required this.parts,
    this.rating,
