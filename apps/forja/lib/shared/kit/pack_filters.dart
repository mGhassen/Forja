...
class PackFiltersRegistry {
  ...
  static List<PlayFilterSpec> playFiltersFor(String pluginId) {
...
class _PackFilters {
  ...
  final List<PlayFilterSpec> play;
  ...
  factory _PackFilters.fromJson(Map<String, dynamic> json) {
    ...
    final play = <PlayFilterSpec>[];
    final rawPlay = json['play'];
    if (rawPlay is List) {
      for (final row in rawPlay) {
        if (row is! Map) continue;
        play.add(PlayFilterSpec.fromJson(Map<String, dynamic>.from(row)));
      }
    }

    var menus = <ChromeMenuItem>[];
...
ChromeMenuItem? _menuFromJson(Map<String, dynamic> json) {
...
List<ChromeMenuItem> _menusFromLegacyMedia(Map<String, dynamic> media) {
  final out = <ChromeMenuItem>[];
  final films = MetaFilterAst.parse(media['films']);
  if (films != null) {
    out.add(
      ChromeMenuItem(
        id: 'films',
  ...
  final series = MetaFilterAst.parse(media['series']);
  if (series != null) {
