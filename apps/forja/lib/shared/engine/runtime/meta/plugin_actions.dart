...
///
/// Not the JS engine runtime (`shared/engine/runtime/vm/runtime.dart`) — this is the
...
class MetaRuntime {
  ...
  Future<MetaEnvelope> _fetch({
    ...
    if (envelope.notModified && entry != null) {
      EngineCache.instance.touchEntry(key);
      return _pipeEnrichCached(
        cacheKey: key,
        sourcePluginId: pluginId,
        packSourceUrl: packSourceUrl,
  ...
  MetaEnvelope _cachedEnvelope(String action, EngineCacheEntry entry) =>
      MetaEnvelope(
        ok: true,
        action: action,
        data: entry.data,
