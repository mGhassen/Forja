...
import 'package:forja/shared/engine/runtime/catalog_list_open.dart';
...
typedef HubFeedKey = ({String pluginId, String status});

/// MetaRuntime `feed` for any list-typed hub — pack owns composition via
/// `ctx.host.bookmarks` + `ctx.host.simkl` / `ctx.host.liveFeed`.
final hubPluginFeedProvider =
    FutureProvider.autoDispose.family<List<Map<String, dynamic>>, HubFeedKey>((
  ref,
  key,
) async {
  ...
  final liveQuery = LiveFeedQuery.fromHorizonPref(
  ...
  try {
    final env = await MetaRuntime.instance.run(
      ...
      params: {
        ...
        '_rev': revision,
        'catalogFilter': liveQuery.catalogFilter,
        'scheduleStatus': liveQuery.scheduleStatus,
        'scheduleHorizon': liveQuery.scheduleHorizon,
        'sportFilter': liveQuery.sportFilter,
      },
      forceRefresh: true,
    );
    if (env.ok) {
...
final hubPluginCatalogProvider =
    Provider.family<AsyncValue<PluginHubFeedPage>, HubFeedKey>((ref, key) {
...
final class PluginHubFeedListSource extends KitListSource {
  ...
  @override
  List<String> get omitKindIds => const [LiveSurfaceOpen.engineType];

  HubFeedKey _key(String status) => (pluginId: pluginId, status: status);
