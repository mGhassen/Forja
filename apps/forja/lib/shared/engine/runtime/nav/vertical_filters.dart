...
class VerticalFilterOption {
  const VerticalFilterOption({
    required this.id,
    required this.label,
    required this.logo,
  ...
  factory VerticalFilterOption.fromJson(Map<String, dynamic> json) {
    return VerticalFilterOption(
      ...
      filter: MetaFilterAst.parse(json['filter']),
    );
  }
}

/// Layout-declared vertical filter strip for a hub tab.
class VerticalFiltersSpec {
  ...
  final List<VerticalFilterOption> options;

  VerticalFilterOption? optionById(String? id) {
  ...
  factory VerticalFiltersSpec.fromWidget({
    ...
    final options = <VerticalFilterOption>[];
    final raw = widget['options'];
    if (raw is List) {
      for (final o in raw) {
        if (o is! Map) continue;
        final opt = VerticalFilterOption.fromJson(
...
abstract final class VerticalFiltersRegistry {
  ...
  static final Map<String, VerticalFiltersSpec> _specs = {};
  ...
  /// [unregister] runs from [PackLayoutHost.dispose] while the tree is locked.
  ...
  static VerticalFiltersSpec? specFor(String? tabId) {
    final id = tabId?.trim();
    if (id == null || id.isEmpty) return null;
    return _specs[id];
  }
  ...
  static void register(VerticalFiltersSpec spec) {
  ...
  static VerticalFilterOption? selectedOptionFor(String? tabId) {
