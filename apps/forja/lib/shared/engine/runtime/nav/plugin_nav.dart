...
abstract final class PluginNavRegistry {
  ...
  static bool isKitTab(String id) {
    _ensureSeeded();
  ...
  ///
  /// Keeps [coreShellNavIds] + Addons-gated host tabs. When no hub
  ...
  static Future<List<(EnginePack, EnginePlugin, MetaNavSpec)>> listNavHubs({
    bool requireEnabled = true,
  }) async {
    final packs = await EngineService.instance.listPacks();
  ...
  static Future<bool> _refreshBody() async {
    ...
    for (final (pack, pl, nav) in hubs) {
      ...
      if (PackAssets.isPackRelativeAsset(nav.icon)) {
    ...
    if (dests.isNotEmpty || extras.isNotEmpty) {
      // Pack hub tabs only — never auto-show settings / non-hub core.
  ...
  static Future<String?> resolveKitPluginId({
    String? pluginId,
    String? tabId,
    String? engineType,
  }) async {
  ...
  static IconData iconDataFor(MetaNavSpec nav) {
    return ForjaHostAssets.defaultNavIcon;
  }

  static Color? accentFor(MetaNavSpec nav) {
