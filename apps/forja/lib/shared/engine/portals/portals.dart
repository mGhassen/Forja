/// Opaque portal engine — vault inventory, network helpers, M3U, share crypto.
///
/// Product catalog caches / channel search / sports gates live in packs.
/// Guide paint: `forja_foundation/widgets/guide/`. Portal→paint adapters:
/// `engine/portals/guide/`. Player mounts foundation widgets only.
library;

export 'channel_search/match_fixture_keys.dart';
export 'guide/guide.dart';
export 'm3u/m3u_models.dart';
export 'm3u/m3u_parser.dart';
export 'm3u/m3u_store.dart';
export 'models.dart';
export 'network/portal_network.dart';
export 'network/pastesh_decryptor.dart';
export 'share/portal_csv.dart';
export 'share/portal_share.dart';
export 'store/portal_vault_inventory.dart';
export 'store/storage.dart';
export 'portals_host.dart';
