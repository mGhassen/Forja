...
import 'package:forja/shared/engine/unlock/match_event.dart';
import 'package:forja/shared/engine/unlock/schedule_sport_filter.dart';
...
import 'package:rust/rust.dart' show BuiltInPlayerContext, SettingsService, StremioService;

/// Live resolve + Stremio providers + native play (RFC-091).
/// Live TV portal channels: hub `liveTv` → IPTV `searchChannels`.
abstract final class LiveResolveStreams {
  ...
  static Future<List<LivePlaySource>> loadProviders(
    Map<String, dynamic> legacyRow, {
    bool force = false,
    void Function(List<LivePlaySource> partial)? onPartial,
  }) async {
    ...
    if (match.isStremio) {
      List<LivePlaySource> stremioOnly = const [];
      try {
      ...
      if (stremioOnly.isNotEmpty) {
        _providersCache[cacheKey] = _ProvidersCacheEntry(
          ...
          sources: List<LivePlaySource>.from(stremioOnly),
        );
      }
      return stremioOnly;
    }

    var forja = <LivePlaySource>[];
    var stremio = <LivePlaySource>[];

    List<LivePlaySource> merged() {
    ...
    if (out.isNotEmpty) {
      _providersCache[cacheKey] = _ProvidersCacheEntry(
        expiresAt: DateTime.now().add(_providersCacheTtl),
        sources: List<LivePlaySource>.from(out),
      );
    }
    return out;
  }
  ...
  static Future<void> play(
    ...
    required LivePlaySource picked,
    required String title,
    String? subtitle,
  }) async {
    if (!context.mounted) return;
    final kind = picked.liveSourceKind ?? PortalLiveSourceKind.liveEngine;
    ...
    await openForjaLiveNativePlayer(
      ...
      engineContext: kind == PortalLiveSourceKind.iptvXtream ||
  ...
  static Future<List<LivePlaySource>> _loadForjaLiveProviders(
    MatchEvent match, {
    void Function(List<LivePlaySource> partial)? onPartial,
  }) async {
    final plugins =
  ...
  static LivePlaySource _choiceToPanelSource(_StreamChoice choice) {
    ...
    return LivePlaySource(
      ...
      liveSourceKind: PortalLiveSourceKind.liveEngine,
  ...
  static Future<List<LivePlaySource>> _loadStremioProviders(
    MatchEvent match,
  ) async {
    if (match.isStremio) {
  ...
  static Future<List<LivePlaySource>> _stremioPlaySourcesFor(
    MatchEvent match,
  ) async {
    final out = <LivePlaySource>[];
  ...
  static Future<LivePlaySource?> resolveLiveEngineSource(
    LivePlaySource catalog, {
  ...
  static Future<LivePlaySource?> _resolveStreamToEnginePlaySource(
    MatchEvent match,
    MatchStream stream, {
    void Function(String)? onProgress,
  ...
  static Future<LivePlaySource?> _unlockForjaLiveSourceRef(
    MatchEvent match,
    MatchStream stream, {
    void Function(String)? onProgress,
  }) async {
  ...
  static LivePlaySource _liveEnginePlaySource({
    required MatchEvent match,
