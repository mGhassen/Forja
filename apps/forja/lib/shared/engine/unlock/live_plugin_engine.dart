...
class LivePluginEngine {
  ...
  static void _cachePluginMeta(EnginePlugin plugin) {
    ...
    _scheduleEnrichByPluginId[key] =
        isScheduleEnrichConfig(plugin.config);
  ...
  static bool isIframeLiveConfig(Map<String, dynamic> config) =>
      catalogLayoutFromConfig(config) == 'iframe';

  static bool isIframeLivePlugin(EnginePlugin plugin) =>
      isIframeLiveConfig(plugin.config);

  static bool cachedIsIframeLive(String pluginId) {
  ...
  static bool cachedLiveFeedUncapped(String pluginId) {
    if (pluginId.isEmpty) return false;
  ...
  static bool isScheduleEnrichConfig(Map<String, dynamic> config) {
    if (!scheduleFullDayFromConfig(config)) return false;
    final leagues = config['leagues'];
    return leagues is List && leagues.isNotEmpty;
  }

  static bool isScheduleEnrichPlugin(EnginePlugin plugin) =>
      isScheduleEnrichConfig(plugin.config);

  static bool cachedIsScheduleEnrich(String pluginId) {
    if (pluginId.isEmpty) return false;
  ...
  /// Cached mirror of [isProviderStreamFeed] for schedule soft-match.
  ...
  static bool isProviderStreamFeed(EnginePlugin plugin) {
    if (!plugin.supportsLiveFeed) return false;
    if (isScheduleEnrichPlugin(plugin)) return false;
  ...
  static Future<void> warmIframeLiveWebOrigin() async {
  ...
  static Future<void> warmPluginMeta() async {
    ...
    for (final p
        in await EngineService.instance.listEnabledLiveFeedPlugins()) {
      _cachePluginMeta(p);
    }
    // Disabled catalogs still carry config for warm cache.
    for (final pack in await EngineService.instance.listPacks()) {
      for (final p in pack.plugins) {
        if (p.isLiveSportPlugin || p.isLiveFeedPlugin) _cachePluginMeta(p);
      }
    }
  }
  ...
  static Future<List<EnginePlugin>> enabledIframeLivePlugins() async {
    await warmPluginMeta();
    final out = <EnginePlugin>[];
    for (final p in await EngineService.instance.listEnabledLiveFeedPlugins()) {
      if (isIframeLivePlugin(p)) out.add(p);
    }
    return out;
  }

  /// Enabled schedule catalogs that link to a live resolve plugin (`providerId`).
  static Future<List<EnginePlugin>> enabledScheduleStreamPlugins() async {
    await warmPluginMeta();
    final out = <EnginePlugin>[];
    for (final p in await EngineService.instance.listEnabledLiveFeedPlugins()) {
      if (!isProviderStreamFeed(p) || isIframeLivePlugin(p)) continue;
      out.add(p);
    }
    return out;
  }
  ...
  static Future<String> iframeLiveWebOrigin() async {
    await warmPluginMeta();
    for (final e in _catalogLayoutByPluginId.entries) {
      if (e.value != 'iframe') continue;
      final o = await pluginWebOrigin(e.key);
  ...
  static Future<String> iframeLiveWebReferer() async =>
      _refererForOrigin(await iframeLiveWebOrigin());

  /// Sync label for iframe-layout catalogs after [warmPluginMeta].
  static String iframeLiveHostLabelCached() {
    for (final e in _catalogLayoutByPluginId.entries) {
      if (e.value != 'iframe') continue;
  ...
  static String scheduleStreamHostLabelCached() {
    for (final e in _catalogLayoutByPluginId.entries) {
      if (e.value == 'iframe') continue;
  ...
  static Future<List<Map<String, dynamic>>> fetchLiveFeedPlugin(
    ...
    return EngineService.instance.runLiveFeed(catalogPlugin: catalogPlugin);
  }
  ...
  static Future<LiveEngineResolveResult?> resolve({
    ...
    // Pack JS owns /fetch + crack via ctx.live.goatUnlock / gasmUnlock /
    // sportsEmbedUnlock. Host only runs those opaque bridges — never
    // switch(nativeUnlock) on streamed/ppv/watchfooty in Dart.
    ...
    final pluginDirect = first['directPlayback'] == true;
    return LiveEngineResolveResult.playable(
      url: url,
      headers: headers,
      label: (first['name'] ?? first['title'] ?? label).toString(),
      directPlayback: liveEngineOpenDirect(url, pluginDirect: pluginDirect),
    );
  }
...
bool liveEnginePreferDirectPlayback(String m3u8Url) {
  // Pack sets `directPlayback` on resolve rows. Host only knows MediaKit
  // transport limits (path-signed / S3), not which pack CDN is which.
  return false;
}

/// Trust pack `directPlayback`. Force `/hls-proxy` for CDNs that need host
/// TLS + Referer rewrite on child playlists (MediaKit constraint).
