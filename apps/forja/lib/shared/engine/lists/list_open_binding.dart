...
/// Engine-type tokens implied by a list row (pack-agnostic).
@visibleForTesting
Set<String> listOpenIdentityTokens(
  ...
  if (mt == 'asian_drama' || mt == 'drama') tokens.add('drama');
  if (mt == 'anime') tokens.add('anime');
  if (mt == 'movie') tokens.add('movie');
  if (mt == 'tv' || mt == 'series') tokens.add('tv');

  final kind = item['kind']?.toString() ?? '';
  if (kind == 'asian_drama' || kind == 'drama') tokens.add('drama');
  if (kind == 'anime') tokens.add('anime');
...
abstract final class ListOpenBinding {
  ...
  static Future<({String pluginId, MetaItem meta})?> resolveStored({
    ...
    final hub = legacyListHubEngineType(item);
    if (hub != null) {
      final resolved = await PluginNavRegistry.pluginIdForEngineType(hub);
      if (resolved != null &&
          resolved.isNotEmpty &&
  ...
  static MetaOpen? metaOpenForCandidate(
    ...
    final tokens = listOpenIdentityTokens(item, meta);
    String? surface;
    if (hubTypes.contains('drama') && tokens.contains('drama')) {
      surface = 'drama';
    } else if (hubTypes.contains('anime') && tokens.contains('anime')) {
      surface = 'anime';
    } else if (hubTypes.contains('movie') && tokens.contains('movie')) {
      surface = 'tmdb';
    } else if (hubTypes.contains('tv') && tokens.contains('tv')) {
      surface = 'tmdb';
    } else if (hubTypes.contains('arabic') && tokens.contains('arabic')) {
      surface = 'arabic';
    } else {
      // Prefer existing open when it already matches a hub type.
    ...
    // Rebuild via legacy helper with forced hub mediaType.
    final patched = Map<String, dynamic>.from(item);
    if (surface == 'drama') {
      patched['mediaType'] = 'asian_drama';
      patched['kind'] = 'asian_drama';
    } else if (surface == 'anime') {
      patched['mediaType'] = 'anime';
      patched['kind'] = 'anime';
    }
    return metaOpenFromLegacyListItem(patched);
  }
