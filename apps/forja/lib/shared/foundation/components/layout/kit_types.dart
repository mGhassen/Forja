...
/// id registered via [HostListRegistry] — kit never names products.
abstract final class KitTypes {
  KitTypes._();
...
List<({String id, String label})> kitItemsFromSpec(
...
void walkKitWidgets(
  ...
  void walk(Map<String, dynamic> spec) {
    visit(spec);
    if (!KitTypes.isStack((spec['type'] ?? '').toString())) return;
...
Map<String, Map<String, dynamic>> kitWidgetSpecIndex(
  Iterable<Map<String, dynamic>> roots,
) {
...
void initKitTabSelections(
  ...
  walkKitWidgets(roots, (spec) {
    ...
    if (type != KitTypes.topBar) return;
