...
class AmriExtractor {
  ...
  void _handleSourcesData(dynamic sourcesData) {
    if (_sourcesCompleter != null && !_sourcesCompleter!.isCompleted) {
      try {
        final data = sourcesData is String
            ? jsonDecode(sourcesData)
            : sourcesData;
          ...
          if (msg.startsWith('AMRI_SOURCES:')) {
            try {
              ...
              if (sources != null && sources.isNotEmpty) {
                ...
                final hasUrl = sources.any(
                  (s) =>
                      s['url'] != null || s['file'] != null || s['src'] != null,
                );
                if (hasUrl) {
                  onLog(
                    '📦 Received valid AMRI_SOURCES with ${sources.length} items (URLs found)',
                  );
                  _handleSourcesData(data);
                } else {
                  onLog(
                    '⚠️ Received ${sources.length} sources but NO URLs yet. Waiting...',
                  );
                }
              }
            } catch (e) {
      ...
      final sourcesData = await _sourcesCompleter!.future.timeout(
        ...
        onTimeout: () {
          throw TimeoutException(
            'Automation timed out after ${timeout.inSeconds} seconds',
          );
        },
      );
      if (cancelled()) return {'sources': []};
