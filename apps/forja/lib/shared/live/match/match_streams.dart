...
abstract final class MatchStreams {
      ...
      for (var i = 0; i < catalogMeta.length; i++) {
        ...
        out.add(
          MatchStream(
            ...
            hd: meta.hd,
            embedUrl: embed,
            source: meta.source.trim().isNotEmpty ? meta.source : pluginSource,
  ...
  static MatchStream _streamFromResolveRow({
    ...
    MatchStream? catalogMeta,
    required int index,
  }) {
    final rowViewers = parseLiveViewerCount(row['viewers']);
    final nameRaw =
        (row['name'] ?? row['title'] ?? catalogMeta?.language ?? '').toString();
  ...
  static Map<String, dynamic> _liveEngineResolveParams(
    ...
    return {
      ...
      'isMut': match.isMut,
      'isIframeCatalog': LiveMatchesEngine.cachedIsIframeCatalog(
        match.livePluginId,
      ),
      'hd': stream.hd,
