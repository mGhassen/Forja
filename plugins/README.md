...
Every `manifest.json` under this tree is an installable pack (except `archived/`). The web Community Packs catalog is generated by scanning those manifests — run `node scripts/generate-plugin-catalog.mjs` (or `npm run catalog:generate` in `apps/web`) after adding or removing a pack. Public `catalog.json` is metadata-only; install URLs are generated separately and never shown in the UI. Optional `FORJA_PLUGIN_PACKS_BASE` overrides the install host (default GitHub raw).
