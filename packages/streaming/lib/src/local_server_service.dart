...
class LocalServerService {
  ...
  String get _rustProxyBase => _rustProxyPort > 0
      ? 'http://127.0.0.1:$_rustProxyPort'
      : baseUrl;
  ...
  Future<Response> _handleProxyRequest(Request request) async {
    final params = request.url.queryParameters;
    if (params['url'] == null) {
      return Response.notFound('Missing url parameter');
    }
    if (_rustProxyPort <= 0) {
      return Response(503, body: 'Rust proxy not running');
    }
    return _forwardToRustProxy(request, params, '/proxy');
  }

  Future<Response> _forwardToRustProxy(
    Request request,
  ...
  /// HLS proxy URL — served by Rust engine (`/hls-proxy` on `_rustProxyPort`).
  String getHlsProxyUrl(String targetUrl, Map<String, String> headers,
      {String? stripMode}) {
    final base = '$_rustProxyBase/hls-proxy'
        '?url=${Uri.encodeComponent(targetUrl)}'
