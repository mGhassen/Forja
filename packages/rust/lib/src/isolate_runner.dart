...
Future<String> runHttpGetJson(
  ...
  int timeoutSecs = 15,
  String headersJson = '{}',
}) => EngineJobs.run(EngineAsyncJob.httpGet, {
  'url': url,
  'timeout_secs': timeoutSecs,
...
Future<String> runHttpPostJson(
  ...
  String headersJson = '{}',
  String body = '',
}) => EngineJobs.run(EngineAsyncJob.httpPost, {
  'url': url,
  'timeout_secs': timeoutSecs,
  'headers_json': headersJson,
  'body': body,
...
Future<String> runSubtitleRequestJson(String requestJson) =>
    EngineWorkerPool.run(EngineJobKind.subtitleRequest, {
      'requestJson': requestJson,
    });

Future<String> runTmdbGetJson(String resourcePath, {int timeoutSecs = 15}) =>
    EngineWorkerPool.run(EngineJobKind.tmdbGet, {
...
Future<String> runOpensslAesDecryptJson(
  String intermediate, {
  String passphrase = '',
}) => EngineWorkerPool.run(EngineJobKind.opensslAesDecrypt, {
  'b64': intermediate,
...
}) => EngineWorkerPool.run(EngineJobKind.filterTorrents, {
...
Future<String> runSortTorrentsJson(String resultsJson, String preference) =>
    EngineWorkerPool.run(EngineJobKind.sortTorrents, {
      'resultsJson': resultsJson,
      'preference': preference,
    });

Future<String> runParseM3uJson(String content) =>
    EngineWorkerPool.run(EngineJobKind.parseM3u, {'content': content});

Future<String> runParseHlsMasterJson(String masterUrl, String body) =>
    EngineWorkerPool.run(EngineJobKind.parseHlsMaster, {
      'masterUrl': masterUrl,
      'body': body,
    });
...
Future<String> runParseXtreamStreamsJson(String json, String section) =>
    EngineWorkerPool.run(EngineJobKind.parseXtreamStreams, {
      'json': json,
      'section': section,
    });

Future<String> runParseXtreamSeriesEpisodesJson(String json) =>
    EngineWorkerPool.run(EngineJobKind.parseXtreamSeriesEpisodes, {
      'json': json,
    });
...
Future<String> runDecryptPasteResponse(
  String urlWithHash,
  String rawResponse,
) => EngineWorkerPool.run(EngineJobKind.decryptPasteResponse, {
