...
enum MetaErrorCode {
  ...
  const MetaErrorCode(this.wire);
  final String wire;

  static MetaErrorCode? tryParse(String? raw) {
    if (raw == null) return null;
    final n = raw.trim().toUpperCase().replaceAll('-', '_');
    for (final c in MetaErrorCode.values) {
...
class MetaError {
  ...
  factory MetaError.fromJson(Map<String, dynamic> j) {
    final code = MetaErrorCode.tryParse(j['code']?.toString()) ??
...
class MetaEnvelope {
  ...
  List<MetaItem> get items {
    final raw = data?['items'];
    if (raw is! List) return const [];
    return [
      for (final e in raw)
        if (e is Map) MetaItem.fromJson(Map<String, dynamic>.from(e)),
    ];
  }
  ...
  List<MetaStream> get streams {
    final raw = data?['streams'];
    if (raw is! List) return const [];
    return [
      for (final e in raw)
        if (e is Map) MetaStream.fromJson(Map<String, dynamic>.from(e)),
    ];
  }

  factory MetaEnvelope.failure(
    MetaErrorCode code, {
    String message = '',
    String action = '',
  }) {
    return MetaEnvelope(
      ok: false,
      action: action,
      error: MetaError(code: code, message: message),
    );
  }

  factory MetaEnvelope.fromJson(Map<String, dynamic> j) {
    final err = j['error'];
...
MetaEnvelope? parseEnvelope(dynamic raw) {
  if (raw is Map) {
    final m = Map<String, dynamic>.from(raw);
    if (!m.containsKey('ok')) return null;
    return MetaEnvelope.fromJson(m);
  }
...
/// Engine extract inputs declared by a hub pack on [MetaOpen.extract].
///
/// Host passes [ctx] through to the engine boundary without interpreting keys.
class MetaOpenExtract {
  const MetaOpenExtract({
  ...
  static MetaOpenExtract? fromJson(dynamic raw) {
    if (raw is! Map) return null;
    final m = Map<String, dynamic>.from(raw);
    final resolveType =
  ...
  static MetaOpenExtract fallbackFor({
...
class MetaStream {
  ...
  factory MetaStream.fromJson(Map<String, dynamic> j) => MetaStream(
        url: (j['url'] ?? '').toString(),
  ...
  factory MetaItem.fromJson(Map<String, dynamic> j) {
    ...
    return MetaItem(
      ...
      viewers: () {
        ...
        return _parseViewerCount(j['viewers']);
      }(),
      mode: () {
...
/// Opaque viewer-count parse (string or num). Sport-agnostic; no host deps.
int _parseViewerCount(dynamic value) {
