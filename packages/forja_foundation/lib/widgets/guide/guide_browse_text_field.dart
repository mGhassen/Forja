...
class GuideBrowseTextField extends StatefulWidget {
  const GuideBrowseTextField({
    super.key,
    ...
    this.caretHeight,
    /// When true, D-pad land is browse-only until activate opens the IME.
    this.tvBrowse = false,
  });
  ...
  @override
  State<GuideBrowseTextField> createState() => GuideBrowseTextFieldState();
}

class GuideBrowseTextFieldState extends State<GuideBrowseTextField> {
  bool _editing = false;
  FocusOnKeyEventCallback? _previousKeyHandler;

  bool get _tvBrowse => widget.tvBrowse;
  ...
  @override
  void didUpdateWidget(covariant GuideBrowseTextField oldWidget) {
  ...
  KeyEventResult _handleKey(FocusNode node, KeyEvent event) {
    _ensureKeyHandler();
    if (_browseOnly && guideIsActivateKey(event)) {
      _beginEditing();
