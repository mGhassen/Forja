...
import 'package:forja_foundation/widgets/guide/guide_tv_hold_accel.dart';
...
class ChannelGuidePanel extends StatefulWidget {
  ...
  /// True leanback (no mouse) — skip hover EPG, snap animations, etc.
  final bool leanbackOnly;

  /// Desktop window chrome — full-height dock vs phone floating overlay.
  final bool isDesktop;

  /// Optional panel insets; when null, derived from [isTv] / [isDesktop].
  final EdgeInsets Function(BuildContext)? panelInsets;

  /// Resolve a playable URL for health probes (optional).
  ...
  /// Load opaque EPG programmes for a channel (host maps portal entries).
  final Future<List<GuideEpgProgramme>> Function(GuideChannel)? loadEpg;
...
class _ChannelGuidePanelState extends State<ChannelGuidePanel> {
  ...
  /// Desktop hover highlight — paint only; does not change focus or channel list.
  ...
  bool _jumpListToIndex(
    ...
    final margin = ChannelGuidePanel.listFocusMargin;
    ...
    final useAnimate = animate && !widget.isTv;
    if (useAnimate) {
  ...
  @override
  void didUpdateWidget(covariant ChannelGuidePanel oldWidget) {
  ...
  void _revealChannelLogo(String channelId) {
    if (!widget.leanbackOnly || channelId.isEmpty) return;
  ...
  bool _showChannelLogo(GuideChannel channel) {
    if (!widget.leanbackOnly) return true;
  ...
  void _focusGroupAt(
    ...
    if (next == _focusedGroupIndex && _hoveredGroupIndex == null) {
  ...
  double _panelHeight(BuildContext context) {
    ...
    if (widget.isTv) return available;
    // Desktop: full height to the bottom. Phone: capped floating overlay.
    if (widget.isDesktop) return available;
    final target =
        available * ChannelGuidePanel.panelHeightFractionPhone;
    return target.clamp(280.0, available).toDouble();
  }

  GuideChannel? get _currentChannel {
  ...
  Future<void> _runHealthCheck(GuideChannel ch) async {
    ...
    if (probe == null && widget.resolvePlayUrl == null &&
    ...
    try {
      ...
      if (probe != null) {
        ...
        // No host probe — treat resolved URL as alive.
        ok = true;
      }
      if (!mounted) return;
  ...
  List<GuideChannel> get _visibleChannels =>
      widget.guide.channelsForGroup(_browseGroupId);

  GuideChannel? get _epgPeekChannel {
  ...
  Future<List<GuideEpgProgramme>>? _epgFutureFor(GuideChannel ch) {
    final load = widget.loadEpg;
    if (!widget.epgEnabled || load == null) return null;
    return load(ch);
  }
  ...
  void _scheduleHoverEpg(GuideChannel ch) {
    if (!widget.epgEnabled || widget.leanbackOnly) return;
    _epgHoverTimer?.cancel();
  ...
  KeyEventResult _onKey(FocusNode node, KeyEvent event) {
    ...
    if (key == LogicalKeyboardKey.arrowUp) {
      _moveFocus(-GuideTvHoldAccel.lastStep);
      return KeyEventResult.handled;
    }
          ...
          children: [
            ...
            if (tv)
              Positioned(
                ...
                top: 0,
                bottom: 0,
                child: _buildPanelWithEpgPeek(wide: wide, tvRail: true),
              )
              ...
              Positioned(
                left: 0,
                top: ChannelGuidePanel.panelVerticalGap,
                bottom: ChannelGuidePanel.panelEdgeGap,
                child: _buildPanelWithEpgPeek(wide: wide, tvRail: false),
              )
            else
              Align(
  ...
  /// Rail + optional EPG peek as one Row — peek must NOT be a full-height
  /// [Positioned] over the live video (show/hide that layer ANRs on ATV/emu).
    ...
    return ListLetterJumpScope(
      ...
      child: LiveTvScrollbar(
        ...
        itemBuilder: (_, i) {
          ...
          final hovered = _hoveredGroupIndex == i;
          final lit = focused || hovered;
          final hasPlaying = _groupHasPlayingChannel(g.id);
          return MouseRegion(
            onEnter: (_) {
              _cancelHoverEpg();
              // Hover = paint only. Never steal focus column / focused index.
              if (_hoveredGroupIndex == i) return;
              setState(() => _hoveredGroupIndex = i);
            },
            onExit: (_) {
              if (_hoveredGroupIndex != i) return;
