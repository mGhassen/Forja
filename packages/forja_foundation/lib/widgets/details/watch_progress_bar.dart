...
bool watchProgressIsFinished(int positionMs, int durationMs) {
  if (durationMs <= 0) return false;
  return positionMs / durationMs >= kWatchFinishedThreshold;
}
...
class WatchProgressBar extends StatelessWidget {
  ...
  static bool isResumable(int positionMs, int durationMs) =>
      watchProgressIsResumable(positionMs, durationMs);

  static bool isFinished(int positionMs, int durationMs) =>
      watchProgressIsFinished(positionMs, durationMs);
  ...
  Widget build(BuildContext context) {
    ...
    if (!isResumable(positionMs, durationMs) &&
        !isFinished(positionMs, durationMs)) {
