...
/// Extracted palette colors for atmosphere chrome.
...
/// Props-only — host may enrich via palette libs; Zone A keeps fallback.
Future<AtmosphereColors> extractAtmosphereColors(String imageUrl) async {
  if (imageUrl.trim().isEmpty) return AtmosphereColors.fallback;
  return AtmosphereColors.fallback;
}

/// Leading/trailing edge tones for carousel seam blends.
...
/// Props-only stub — host may sample edges; Zone A returns fallback.
Future<BackdropEdgeColors> extractBackdropEdgeColors(String imageUrl) async {
  if (imageUrl.trim().isEmpty) return BackdropEdgeColors.fallback;
...
/// Call [loadAtmosphere] in initState. Pass [enableMotion] from host policy.
