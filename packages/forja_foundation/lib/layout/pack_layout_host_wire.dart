...
import 'package:forja_foundation/widgets/chrome/epg_guide.dart';
...
class _KitListEventSearchState extends ConsumerState<KitListEventSearch>
  ...
  void _focusField({bool edit = false}) {
    ...
    if (!edit) {
      (_fieldKey.currentState as dynamic)?.endEditing(keepFocus: true);
      return;
    }
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      (_fieldKey.currentState as dynamic)?.beginEditing();
    });
  }
  ...
  KeyEventResult _onFieldKey(FocusNode node, KeyEvent event) {
    ...
    if (event is! KeyDownEvent) return KeyEventResult.ignored;
    final editing = ((_fieldKey.currentState as dynamic)?.isEditing as bool?) ?? false;
    final key = event.logicalKey;
...
final kitTopBarCatalogOptionsProvider = FutureProvider.autoDispose
  ...
  for (final d in depsKey.split(',')) {
    ...
    switch (token) {
      case 'stremio':
        ref.watch(addonRevisionProvider as dynamic);
      default:
        break;
    }
  }
    ...
    if (isView) {
      ...
      if (rawItems is List) {
        for (final e in rawItems) {
          ...
          if (!cycle.contains(id)) cycle.add(id);
        }
      }
      if (cycle.isEmpty) cycle.addAll(const ['cards', 'list']);
      var cur = override;
      if (cur.isEmpty || !cycle.contains(cur)) cur = cycle.first;
...
class _PluginKitTopBarState extends State<PluginKitTopBar> {
  ...
  Future<void> _loadPluginAndFilters(String pluginId) async {
    ...
    setState(() => _plugin = found);
    if (found?.hasCapability(PackCapabilities.filters) == true) {
  ...
  PackPluginSnapshot? get _pluginForTab {
    final pluginId = PluginNavRegistry.pluginIdForTabSync(widget.tabId);
    if (pluginId == null || _plugin == null) return null;
    return _plugin!.id == pluginId ? _plugin : null;
  }

  bool _layoutOnlyHub(PackPluginSnapshot? plugin) {
  ...
  Widget _buildBrowseTopBar(BuildContext context) {
    ...
    final label =
        (PluginNavRegistry.destinations[widget.tabId] as dynamic)?.label as String? ?? 'Search';
      ...
      if (source != null) {
        ...
        if (page != null) {
          ...
          for (final e in page.entriesForKind(null)) {
            ...
            if (!found.contains(k)) found.add(k);
          ...
          // Pack controls order (pinned cats first via feed row order).
          final sorted = found;
      ...
      return ShellMoodCircleItem(
        ...
        onTap: () {
          if (!on) {
            ...
            edges?.onSelectAlreadySelected?.call();
          }
        },
...
class _KitListWidgetState extends ConsumerState<KitListWidget> {
    ...
    return pageAsync.when(
      ...
      data: (page) {
        ...
        if (_isTimeline) {
          ...
          body = _grid(context, source, entries);
        }
  ...
  Widget _timelineGuide(
    ...
    for (final e in entries) {
      final id = e.meta.id.trim();
      if (id.isEmpty) continue;
      final title = e.meta.name.trim().isEmpty ? id : e.meta.name.trim();
      final logo = (e.meta.poster.trim().isNotEmpty
              ? e.meta.poster
              : (e.legacyRow['logo'] ?? '').toString())
          .trim();
      ...
      final raw = e.legacyRow['programmes'] ?? e.legacyRow['epg'];
      if (raw is! List) continue;
      for (final p in raw) {
    ...
    final windowStart = now - const Duration(hours: 1).inMilliseconds;
    final windowEnd = now + const Duration(hours: 5).inMilliseconds;
    final byId = {for (final e in entries) e.meta.id: e};
    return Padding(
      padding: EdgeInsets.only(
        left: ShellTokens.compactChromeLeadingInset(context),
        right: ShellTokens.bodyHorizontalPadding,
        top: 4 + _hoistedTopBarInset(context),
        bottom: shellTvKitScrollBottomGap(context),
      ),
      child: EpgGuide(
        channels: channels,
